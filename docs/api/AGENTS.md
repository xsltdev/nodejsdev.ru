...
-   Во внутренних ссылках можно переводить только видимый текст. Сам fragment после `#` должен в точности совпадать с существующим anchor id из заголовка или явного `{: #... }`; не русифицируй его по тексту заголовка.
...
-   Если у заголовка есть явный anchor id, оформляй его инлайн в той же строке заголовка: `## Заголовок {#anchor-id}`. Не оставляй связанный с заголовком anchor отдельным блоком на следующей строке вида `{: #anchor-id}`.
