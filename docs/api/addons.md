---
description: Аддоны Node.js – это динамически подключаемые объекты, написанные на C или С++, которые могут быть загружены в Node.js посредством функции require() и использованы так, как будто они являются обычным модулем Node.js
---
