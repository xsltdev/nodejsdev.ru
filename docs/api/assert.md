...
В режиме строгого утверждения нестрогие методы ведут себя как соответствующие им строгие методы. Например, [`assert.deepEqual()`][`assert.deepequal()`] будет вести себя как [`assert.deepStrictEqual()`][`assert.deepstrictequal()`].
...
-   **string**: Используется как есть. Если после строки `message` указаны дополнительные аргументы, они обрабатываются как подстановки в стиле printf (см. [`util.format()`][`util.format()`]).
...
Чтобы отключить цвета, используйте переменные окружения `NO_COLOR` или `NODE_DISABLE_COLORS`. Это также отключит цвета в REPL. Подробнее о поддержке цветов в терминальных средах см. документацию по tty [`getColorDepth()`][`getcolordepth()`].

## Устаревший режим утверждения

Устаревший режим утверждения использует оператор [`==` operator][`==` operator] в:
...
Устаревший режим утверждения может давать неожиданные результаты, особенно при использовании [`assert.deepEqual()`][`assert.deepequal()`]:
...
-   `actual` {any} Устанавливается в аргумент `actual` для таких методов, как [`assert.strictEqual()`][`assert.strictequal()`].
-   `expected` {any} Устанавливается в ожидаемое значение для таких методов, как [`assert.strictEqual()`][`assert.strictequal()`].
...
Псевдоним [`assert.deepStrictEqual()`][`assert.deepstrictequal()`].

**Режим унаследованных утверждений (legacy assertion mode)**

> Стабильность: 3 - Legacy: вместо этого используйте [`assert.deepStrictEqual()`][`assert.deepstrictequal()`].

Проверяет глубокое равенство между параметрами `actual` и `expected`. Предпочтительно использовать [`assert.deepStrictEqual()`][`assert.deepstrictequal()`] - у [`assert.deepEqual()`][`assert.deepequal()`] результат может быть неожиданным.
...
-   Примитивы сравниваются оператором [`==`][`==` operator], за исключением {NaN}: если с обеих сторон {NaN}, они считаются совпадающими.
-   [Теги типов][object.prototype.tostring()] объектов должны совпадать.
-   Учитываются только [перечислимые собственные свойства][enumerable "own" properties].
-   При наличии сравниваются конструкторы объектов.
-   Имена, сообщения, причины (cause) и свойство errors у [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error) всегда сравниваются, даже если это не перечислимые свойства.
-   [Обёртки над примитивами][object wrappers] сравниваются и как объекты, и как развёрнутые значения.
...
-   Реализация не проверяет [`[[Prototype]]`][prototype-spec] объектов.
...
Следующий пример не выбрасывает [`AssertionError`][`assertionerror`], потому что примитивы сравниваются оператором [`==`][`==` operator].
...
-   Примитивы сравниваются через [`Object.is()`][`object.is()`].
-   [Теги типов][object.prototype.tostring()] объектов должны совпадать.
-   [`[[Prototype]]`][prototype-spec] объектов сравнивается оператором [`===` operator][`===` operator].
-   Учитываются только [перечислимые собственные свойства][enumerable "own" properties].
-   Имена, сообщения, причины (cause) и свойство errors у [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error) всегда сравниваются, даже если это не перечислимые свойства. Сравнивается и свойство `errors`.
-   Сравниваются и перечислимые собственные свойства [Symbol](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol).
-   [Обёртки над примитивами][object wrappers] сравниваются и как объекты, и как развёрнутые значения.
-   Свойства объектов `Object` сравниваются без учёта порядка.
-   Ключи [Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) и элементы [Set](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set) сравниваются без учёта порядка.
-   Рекурсия останавливается, когда значения различаются или когда одна из сторон натыкается на циклическую ссылку.
...
Если строка совпадает с шаблоном или аргумент `string` имеет тип, отличный от `string`, выбрасывается [`AssertionError`][`assertionerror`]: свойство `message` равно значению параметра `message`. Если `message` не передан, подставляется сообщение по умолчанию. Если `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), выбрасывается он, а не [`AssertionError`][`assertionerror`].
...
При необходимости параметр `error` может быть [`Class`][`class`], [RegExp](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions) или функцией проверки. Подробнее - в [`assert.throws()`][`assert.throws()`].

Поведение после ожидания завершения совпадает с [`assert.doesNotThrow()`][`assert.doesnotthrow()`], кроме асинхронности.
...
Если ошибка выброшена и её тип совпадает с типом, заданным параметром `error`, выбрасывается [`AssertionError`][`assertionerror`]. Если тип другой или параметр `error` не задан, ошибка пробрасывается вызывающему коду.

При необходимости `error` может быть [`Class`][`class`], [RegExp](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions) или функцией проверки. Подробнее - в [`assert.throws()`][`assert.throws()`].
...
Следующий пример приводит к [`AssertionError`][`assertionerror`] с сообщением «Got unwanted exception...»:
...
Если выброшена [`AssertionError`][`assertionerror`] и для параметра `message` задано значение, оно дописывается к сообщению [`AssertionError`][`assertionerror`]:
...
Псевдоним [`assert.strictEqual()`][`assert.strictequal()`].

**Режим унаследованных утверждений (legacy assertion mode)**

> Стабильность: 3 - Legacy: вместо этого используйте [`assert.strictEqual()`][`assert.strictequal()`].

Проверяет неглубокое равенство с приведением типов между `actual` и `expected` с помощью оператора [`==`][`==` operator]. `NaN` обрабатывается особым образом: считается совпадающим, если `NaN` с обеих сторон.
...
Выбрасывает [`AssertionError`][`assertionerror`] с указанным сообщением или сообщением по умолчанию. Если `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), выбрасывается он, а не [`AssertionError`][`assertionerror`].
...
changes:
  - version: v10.0.0
    ...
    description: Instead of throwing the original error it is now wrapped into
                 an [`AssertionError`][`assertionerror`] that contains the full stack trace.
  - version: v10.0.0
    pr-url: https://github.com/nodejs/node/pull/18247
...
??? note "История"
    ...
    | Версия | Изменения |
    | --- | --- |
    | v10.0.0 | Вместо того, чтобы выдавать исходную ошибку, она теперь заключена в [`AssertionError`][`assertionerror`], который содержит полную трассировку стека. |
...
Если строка не совпадает с шаблоном или аргумент `string` имеет тип, отличный от `string`, выбрасывается [`AssertionError`][`assertionerror`]: свойство `message` равно значению параметра `message`. Если `message` не передан, подставляется сообщение по умолчанию. Если `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), выбрасывается он, а не [`AssertionError`][`assertionerror`].
...
Псевдоним [`assert.notDeepStrictEqual()`][`assert.notdeepstrictequal()`].

**Режим утверждения Legacy**

> Стабильность: 3 - Legacy: вместо этого используйте [`assert.notDeepStrictEqual()`][`assert.notdeepstrictequal()`].

Проверяет произвольное глубокое неравенство. Противоположность [`assert.deepEqual()`][`assert.deepequal()`].
...
Проверяет глубокое строгое неравенство. Противоположность [`assert.deepStrictEqual()`][`assert.deepstrictequal()`].
...
Если значения глубоко и строго равны, выбрасывается [`AssertionError`][`assertionerror`] со свойством `message`, равным значению параметра `message`. Если параметр `message` имеет значение `undefined`, подставляется сообщение об ошибке по умолчанию. Если параметр `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), он будет выброшен вместо [`AssertionError`][`assertionerror`].
...
Псевдоним [`assert.notStrictEqual()`][`assert.notstrictequal()`].

**Режим утверждения Legacy**

> Стабильность: 3 - Legacy: вместо этого используйте [`assert.notStrictEqual()`][`assert.notstrictequal()`].

Проверяет неглубокое принудительное неравенство с помощью [`!=` operator][`!=` operator]. `NaN` обрабатывается особым образом и считается совпадающим, если обе стороны - `NaN`.
...
Если значения равны, выбрасывается [`AssertionError`][`assertionerror`] со свойством `message`, равным значению параметра `message`. Если параметр `message` имеет значение `undefined`, подставляется сообщение об ошибке по умолчанию. Если параметр `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), он будет выброшен вместо `AssertionError`.
...
Проверяет строгое неравенство между параметрами `actual` и `expected`, определяемое [`Object.is()`][`object.is()`].
...
Если значения строго равны, выбрасывается [`AssertionError`][`assertionerror`] со свойством `message`, равным значению параметра `message`. Если параметр `message` имеет значение `undefined`, подставляется сообщение об ошибке по умолчанию. Если параметр `message` является экземпляром [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error), он будет выброшен вместо `AssertionError`.
...
Если `asyncFn` - функция и она синхронно выбрасывает ошибку, `assert.rejects()` вернёт отклонённый `Promise` с этой ошибкой. Если функция не возвращает промис, `assert.rejects()` вернёт отклонённый `Promise` с ошибкой [`ERR_INVALID_RETURN_VALUE`][`err_invalid_return_value`]. В обоих случаях обработчик ошибки пропускается.

Помимо асинхронного ожидания завершения поведение совпадает с [`assert.throws()`][`assert.throws()`].

Если указано, `error` может быть [`Class`][`class`], [RegExp](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions), функцией проверки, объектом, для которого проверяется каждое свойство, или экземпляром ошибки, для которого проверяется каждое свойство, включая неперечислимые свойства `message` и `name`.

Если указано, `message` будет сообщением, которое передаёт [`AssertionError`][`assertionerror`], если `asyncFn` не отклонится.
...
`error` не может быть строкой. Если вторым аргументом передана строка, считается, что `error` опущен, и строка используется как `message`. Это легко даёт незаметные ошибки. Внимательно прочитайте пример в [`assert.throws()`][`assert.throws()`], если рассматриваете передачу строки вторым аргументом.
...
Проверяет строгое равенство между параметрами `actual` и `expected` так, как определяет [`Object.is()`][`object.is()`].
...
Функция должна вернуть `true`, чтобы показать, что все внутренние проверки прошли. Иначе проверка завершится с [`AssertionError`][`assertionerror`].
...
Этот метод всегда проходит те же проверки, что и [`assert.deepStrictEqual()`][`assert.deepstrictequal()`], и ведёт себя как надмножество этой функции.

### Детали сравнения

-   Примитивы сравниваются через [`Object.is()`][`object.is()`].
-   [Теги типов][object.prototype.tostring()] объектов должны совпадать.
-   [`[[Prototype]]`][prototype-spec] объектов не сравниваются.
-   Учитываются только [перечислимые собственные свойства][enumerable "own" properties].
-   Имена, сообщения, причины (`cause`) и ошибки [Error](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error) сравниваются всегда, даже если это неперечислимые свойства. Сравнивается и `errors`.
-   Сравниваются и перечислимые собственные свойства [Symbol](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol).
-   [Обёртки объектов][object wrappers] сравниваются и как объекты, и как развёрнутые значения.
...
[`assertionerror`]: #класс-assertassertionerror
