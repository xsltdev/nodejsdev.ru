...
Ещё нюанс: `before` и `after` вызываются только для цепочек промисов. Промисы не из `then()`/`catch()` не получат `before` и `after`. Подробнее — в документации V8 [PromiseHooks][promisehooks].
