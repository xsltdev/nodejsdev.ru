...
Так можно реализовать локальное хранилище продолжения выполнения без отслеживающей `Map` для метаданных:
...
По умолчанию выполнениям промисов не назначаются `asyncId` из-за относительно высокой стоимости [API интроспекции промисов](v8.md) в V8. Программы на промисах или `async`/`await` по умолчанию не получают корректные идентификаторы выполнения и запуска для контекстов функций обратного вызова промисов.
