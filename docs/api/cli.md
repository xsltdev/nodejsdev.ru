...
??? note "История"
    ...
    | Версия | Изменения |
    | --- | --- |
    | v16.2.0 | Изменение переменной TZ с помощью `process.env.TZ =` также меняет часовой пояс в Windows. |
    | v13.0.0 | Изменение переменной TZ с помощью `process.env.TZ =` меняет часовой пояс в системах POSIX. |
