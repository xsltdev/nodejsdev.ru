...
### `--build-sea=config` {#--build-seaconfig}
...
### `--build-snapshot-config` {#--build-snapshot-config}
...
### `-C condition`, `--conditions=condition` {#-c condition---conditionscondition}
...
### `--diagnostic-dir=directory` {#--diagnostic-dirdirectory}
...
### `--disable-proto=mode` {#--disable-protomode}
...
### `--disable-warning=code-or-type` {#--disable-warningcode-or-type}
...
### `--dns-result-order=order` {#--dns-result-orderorder}
...
### `--experimental-config-file=config` {#--experimental-config-fileconfig}
...
### `--experimental-eventsource` {#--experimental-eventsource}
...
### `--experimental-inspector-network-resource` {#--experimental-inspector-network-resource}

> Стабильность: 1.1 - Активная разработка

Включает экспериментальную поддержку сетевых ресурсов инспектора.

### `--experimental-loader=module` {#--experimental-loadermodule}
...
### `--experimental-print-required-tla` {#--experimental-print-required-tla}
...
### `--experimental-quic` {#--experimental-quic}
...
### `--experimental-shadow-realm` {#--experimental-shadow-realm}

Используйте этот флаг для включения поддержки [ShadowRealm](https://github.com/tc39/proposal-shadowrealm).

### `--experimental-storage-inspection` {#--experimental-storage-inspection}
...
### `--experimental-test-coverage` {#--experimental-test-coverage}
...
### `--heapsnapshot-near-heap-limit=max_count` {#--heapsnapshot-near-heap-limitmax_count}
...
### `--heapsnapshot-signal=signal` {#--heapsnapshot-signalsignal}
...
### `--input-type=type` {#--input-typetype}
...
### `--inspect-publish-uid=stderr,http` {#--inspect-publish-uidstderr,http}
...
### `--inspect-wait[=[host:]port]` {#--inspect-wait[[host:]port]}
...
### `--localstorage-file=file` {#--localstorage-filefile}
...
### `--max-http-header-size=size` {#--max-http-header-sizesize}

Указывает максимальный размер HTTP-заголовков в байтах. По умолчанию: 16 KiB.

### `--max-old-space-size-percentage=percentage` {#--max-old-space-size-percentagepercentage}
...
### `--no-experimental-webstorage` {#--no-experimental-webstorage}
...
### `--report-dir=directory`, `--report-directory=directory` {#--report-dir=directory---report-directorydirectory}

Каталог, в котором будет создан отчёт.

### `--report-exclude-env` {#--report-exclude-env}

Если передан `--report-exclude-env`, сгенерированный диагностический отчёт не будет содержать данные `environmentVariables`.

### `--report-exclude-network` {#--report-exclude-network}
...
### `--report-filename=filename` {#--report-filenamefilename}
...
### `--report-on-fatalerror` {#--report-on-fatalerror}
...
### `--report-signal=signal` {#--report-signalsignal}
...
### `--secure-heap-min=n` {#--secure-heap-minn}
...
### `--snapshot-blob=path` {#--snapshot-blobpath}
...
### `--test-coverage-branches=threshold` {#--test-coverage-branchesthreshold}
...
### `--test-coverage-exclude` {#--test-coverage-exclude}
...
### `--test-coverage-functions=threshold` {#--test-coverage-functionsthreshold}
...
### `--test-coverage-include` {#--test-coverage-include}
...
### `--test-coverage-lines=threshold` {#--test-coverage-linesthreshold}
...
### `--test-force-exit` {#--test-force-exit}
...
### `--test-global-setup=module` {#--test-global-setupmodule}
...
### `--test-isolation=mode` {#--test-isolationmode}
...
### `--test-rerun-failures` {#--test-rerun-failures}
...
### `--test-skip-pattern` {#--test-skip-pattern}
...
### `--tls-keylog=file` {#--tls-keylogfile}
...
### `--trace-require-module=mode` {#--trace-require-modulemode}
...
### `--unhandled-rejections=mode` {#--unhandled-rejectionsmode}
...
### `--use-largepages=mode` {#--use-largepagesmode}
...
### `--watch-kill-signal` {#--watch-kill-signal}
...
## Переменные окружения {#environment-variables_1}
...
### `NODE_COMPILE_CACHE=dir` {#node_compile_cachedir}
...
### `NODE_COMPILE_CACHE_PORTABLE=1` {#node_compile_cache_portable1}
...
### `NODE_DEBUG_NATIVE=module[,…]` {#node_debug_nativemodule[,…]}
...
### `NODE_DISABLE_COMPILE_CACHE=1` {#node_disable_compile_cache1}
...
### `NODE_SKIP_PLATFORM_CHECK=value` {#node_skip_platform_checkvalue}
...
### `NODE_USE_SYSTEM_CA=1` {#node_use_system_ca1}
...
### `--harmony-shadow-realm` {#--harmony-shadow-realm}

### `--heap-snapshot-on-oom` {#--heap-snapshot-on-oom}

### `--interpreted-frames-native-stack` {#--interpreted-frames-native-stack}
...
### `--perf-basic-prof` {#--perf-basic-prof}

### `--perf-basic-prof-only-functions` {#--perf-basic-prof-only-functions}

### `--perf-prof` {#--perf-prof}

### `--perf-prof-unwinding-info` {#--perf-prof-unwinding-info}

### `--prof` {#--prof}

### `--security-revert` {#--security-revert}

### `--stack-trace-limit=limit` {#--stack-trace-limitlimit}
