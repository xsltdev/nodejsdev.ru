...
-   `type` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Предполагаемое назначение генерируемого секретного ключа. Сейчас допустимы значения `'hmac'` и `'aes'`.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `length` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Длина ключа в битах. Должна быть больше 0.
        -   Если `type` равен `'hmac'`, минимум — 8, максимум — 2<sup>31</sup>−1. Если значение не кратно 8, сгенерированный ключ будет усечён до `Math.floor(length / 8)`.
        -   Если `type` равен `'aes'`, длина должна быть одной из `128`, `192` или `256`.
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
    -   `key` [`<KeyObject>`](#class-keyobject)

Асинхронно генерирует новый случайный секретный ключ заданной длины `length`. Параметр `type` определяет, какие проверки применяются к `length`.
...
Размер сгенерированного ключа HMAC не должен превышать размер блока используемой хеш-функции. Подробнее см. [`crypto.createHmac()`](#cryptocreatehmacalgorithm-key-options).

### `crypto.generateKeyPair(type, options, callback)`

-   `type` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Тип асимметричного ключа для генерации. См. поддерживаемые [типы асимметричных ключей](#asymmetric-key-types).
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `modulusLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Размер ключа в битах (RSA, DSA).
    ...
    -   `saltLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Минимальная длина соли в байтах (RSA-PSS).
    -   `divisorLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Размер `q` в битах (DSA).
    -   `namedCurve` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Имя используемой кривой (EC).
    -   `prime` [`<Buffer>`](buffer.md#buffer) Параметр простого числа (DH).
    -   `primeLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Длина простого числа в битах (DH).
    ...
    -   `paramEncoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Должно быть `'named'` или `'explicit'` (EC). **По умолчанию:** `'named'`.
...
Генерирует новую пару асимметричных ключей указанного типа `type`. См. поддерживаемые [типы асимметричных ключей](#asymmetric-key-types).

Если указаны `publicKeyEncoding` или `privateKeyEncoding`, функция ведёт себя так, как если бы для результата был вызван [`keyObject.export()`](#keyobjectexportoptions). Иначе соответствующая часть ключа возвращается как [`KeyObject`](#class-keyobject).

Рекомендуется кодировать открытые ключи в формате `'spki'`, а закрытые — в `'pkcs8'` с шифрованием для длительного хранения:
...
По завершении будет вызван `callback` с `err`, равным `undefined`, и `publicKey` / `privateKey`, представляющими сгенерированную пару ключей.

Если этот метод вызывается в версии, обёрнутой через [`util.promisify()`](util.md#utilpromisifyoriginal), он возвращает `Promise` с объектом `Object`, у которого есть свойства `publicKey` и `privateKey`.
...
При кодировании открытых ключей рекомендуется использовать `'spki'`. Для закрытых ключей рекомендуется `'pkcs8'` с надёжной парольной фразой; храните парольную фразу в секрете.
...
Возвращаемое значение `{ publicKey, privateKey }` представляет сгенерированную пару ключей. Если было выбрано PEM-кодирование, соответствующий ключ будет строкой, иначе — буфером с данными в кодировке DER.

### `crypto.generateKeySync(type, options)`

-   `type` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Предполагаемое назначение генерируемого секретного ключа. Сейчас допустимы значения `'hmac'` и `'aes'`.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `length` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Длина ключа в битах.
        -   Если `type` равен `'hmac'`, минимум — 8, максимум — 2<sup>31</sup>−1. Если значение не кратно 8, сгенерированный ключ будет усечён до `Math.floor(length / 8)`.
        -   Если `type` равен `'aes'`, длина должна быть одной из `128`, `192` или `256`.
-   Возвращает: [`<KeyObject>`](#class-keyobject)

Синхронно генерирует новый случайный секретный ключ заданной длины `length`. Параметр `type` определяет, какие проверки применяются к `length`.
...
-   `size` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Размер (в битах) генерируемого простого числа.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) При `true` сгенерированное простое число возвращается как `bigint`.
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
...
Если `options.safe` равен `true`, простое число будет безопасным (safe): то есть `(prime - 1) / 2` тоже будет простым.

Параметры `options.add` и `options.rem` позволяют задать дополнительные условия, например для Диффи — Хеллмана:

-   Если заданы и `options.add`, и `options.rem`, простое число удовлетворяет условию `prime % add = rem`.
-   Если задан только `options.add` и `options.safe` не равен `true`, выполняется условие `prime % add = 1`.
-   Если задан только `options.add` и `options.safe` равен `true`, выполняется условие `prime % add = 3`. Это нужно потому, что при `options.add > 2` условие `prime % add = 1` противоречило бы ограничениям, накладываемым `options.safe`.
-   `options.rem` игнорируется, если не указан `options.add`.

И `options.add`, и `options.rem` при передаче в виде `ArrayBuffer`, `SharedArrayBuffer`, `TypedArray`, `Buffer` или `DataView` должны быть закодированы как последовательности в порядке big-endian.

По умолчанию простое число кодируется как последовательность октетов в порядке big-endian в [ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). Если опция `bigint` равна `true`, возвращается [bigint](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/BigInt).

Параметр `size` простого числа напрямую влияет на время генерации: чем больше размер, тем дольше она длится. Поскольку используется функция OpenSSL `BN_generate_prime_ex`, дающая лишь минимальный контроль над возможностью прервать процесс, не рекомендуется генерировать чрезмерно большие простые числа — это может сделать процесс неотзывчивым.
...
-   `nameOrNid` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Имя или NID шифра, о котором нужно получить сведения.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `keyLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Тестовая длина ключа.
...
-   Возвращает: [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `blockSize` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Размер блока шифра в байтах. Свойство опускается, если `mode` равен `'stream'`.
    -   `ivLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Ожидаемая длина вектора инициализации или длина по умолчанию, в байтах. Свойство опускается, если шифр не использует вектор инициализации.
    -   `keyLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Ожидаемая длина ключа или длина по умолчанию, в байтах.
    -   `mode` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Режим шифра. Одно из `'cbc'`, `'ccm'`, `'cfb'`, `'ctr'`, `'ecb'`, `'gcm'`, `'ocb'`, `'ofb'`, `'stream'`, `'wrap'`, `'xts'`.

Возвращает сведения о заданном шифре.

Некоторые шифры допускают ключи и векторы инициализации переменной длины. По умолчанию метод `crypto.getCipherInfo()` возвращает для таких шифров значения по умолчанию. Чтобы проверить, допустимы ли заданные длины ключа и IV для конкретного шифра, используйте опции `keyLength` и `ivLength`. Если переданные значения недопустимы, будет возвращён `undefined`.

### `crypto.getCiphers()`

-   Возвращает: [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Массив имён поддерживаемых алгоритмов шифрования.
...
-   Возвращает: [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Массив имён поддерживаемых эллиптических кривых.
...
Создаёт объект обмена ключами `DiffieHellmanGroup` для предопределённой группы. Поддерживаемые группы перечислены в документации по [`DiffieHellmanGroup`](#class-diffiehellmangroup).

Возвращаемый объект повторяет интерфейс объектов, создаваемых [`crypto.createDiffieHellman()`](#cryptocreatediffiehellmanprime-primeencoding-generator-generatorencoding), но не позволяет менять ключи (например, через [`diffieHellman.setPublicKey()`](#diffiehellmansetpublickeypublickey-encoding)). Преимущество этого метода в том, что сторонам не нужно заранее генерировать и обмениваться модулем группы, что экономит и процессорное время, и трафик.
...
-   Возвращает: [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Массив имён поддерживаемых хеш-алгоритмов, например `'RSA-SHA256'`. Хеш-алгоритмы также называют алгоритмами дайджеста.
...
-   `data` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) Если `data` — строка, она кодируется в UTF-8 перед хешированием. Чтобы для строки использовать другую кодировку входных данных, закодируйте строку в `TypedArray` с помощью `TextEncoder` или `Buffer.from()` и передайте полученный `TypedArray` в этот API.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `outputEncoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) [Кодировка](buffer.md#buffers-and-character-encodings) для представления возвращаемого дайджеста. **По умолчанию:** `'hex'`.
    -   `outputLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Для XOF-функций хеширования, таких как `'shake256'`, опция `outputLength` задаёт желаемую длину выхода в байтах.
