...
[`--pending-deprecation`][`--pending-deprecation`] (или переменной окружения
...
С флагом [`--throw-deprecation`][`--throw-deprecation`] устаревание времени выполнения
приводит к выбросу ошибки. С
[`--pending-deprecation`][`--pending-deprecation`] предупреждения также выдаются для
...
* [`Buffer.allocUnsafe(size)`][alloc_unsafe_size]: создаёт `Buffer` с
  _неинициализированной_ памятью.
* [`Buffer.allocUnsafeSlow(size)`][`Buffer.allocUnsafeSlow(size)`]: создаёт `Buffer` с _неинициализированной_
  памятью.
* [`Buffer.from(array)`][`Buffer.from(array)`]: создаёт `Buffer`, копируя `array`.
* [`Buffer.from(arrayBuffer[, byteOffset[, length]])`][from_arraybuffer] -
  создаёт `Buffer`, оборачивающий указанный `arrayBuffer`.
* [`Buffer.from(buffer)`][`Buffer.from(buffer)`]: создаёт `Buffer`, копируя `buffer`.
...
В методах `spawn()`, `fork()` и `exec()` модуля [`child_process`][`child_process`]
...
[`worker.exitedAfterDisconnect`][`worker.exitedAfterDisconnect`]: старое имя плохо отражало семантику и было
...
Использование API [`crypto.pbkdf2()`][`crypto.pbkdf2()`] без указания `digest` было объявлено устаревшим
...
[`tls.createSecureContext()`][`tls.createSecureContext()`] вместо него.
...
Класс `crypto.Credentials` удалён. Используйте [`tls.SecureContext`][`tls.SecureContext`]
вместо него.
...
Устаревший `String`-интерфейс [`fs.read()`][`fs.read()`] объявлен устаревшим. Вместо него
...
Устаревший `String`-интерфейс [`fs.readSync()`][`fs.readSync()`] объявлен устаревшим. Вместо него
используйте API `Buffer`, как указано в документации.
...
параметр командной строки [`--unhandled-rejections`][`--unhandled-rejections`].
...
удалено. Используйте метод [`Server.getConnections()`][`Server.getConnections()`] вместо него.
...
[`Server.listen({fd: <number>})`][`Server.listen({fd: <number>})`] вместо него.
...
Используйте [`os.tmpdir()`][`os.tmpdir()`] вместо него.
...
[`os.networkInterfaces()`][`os.networkInterfaces()`] вместо него.
...
Модуль `node:sys` устарел. Используйте модуль [`util`][`util`] вместо него.
...
`util.print()` has been removed. Please use [`console.log()`][`console.log()`] instead.
...
`util.puts()` has been removed. Please use [`console.log()`][`console.log()`] instead.
...
`util.debug()` has been removed. Please use [`console.error()`][`console.error()`] instead.
...
`util.error()` has been removed. Please use [`console.error()`][`console.error()`] instead.
...
[`Buffer.allocUnsafeSlow(size)`][`Buffer.allocUnsafeSlow(size)`].
...
Метод [`ecdh.setPublicKey()`][`ecdh.setPublicKey()`] объявлен устаревшим: в API он не несёт пользы.
...
Модуль [`domain`][`domain`] устарел; использовать его не следует.
...
API [`events.listenerCount(emitter, eventName)`][`events.listenerCount(emitter, eventName)`] было устаревшим, так как
дублировало [`emitter.listenerCount(eventName)`][`emitter.listenerCount(eventName)`]. Устаревание отменено: функция
...
API [`fs.exists(path, callback)`][`fs.exists(path, callback)`] устарело. Используйте
[`fs.stat()`][`fs.stat()`] или [`fs.access()`][`fs.access()`].
...
API [`fs.lchmod(path, mode, callback)`][`fs.lchmod(path, mode, callback)`] устарело.
...
API [`fs.lchmodSync(path, mode)`][`fs.lchmodSync(path, mode)`] устарело.
...
API [`fs.lchown(path, uid, gid, callback)`][`fs.lchown(path, uid, gid, callback)`] было устаревшим. Отмена: в libuv
...
API [`fs.lchownSync(path, uid, gid)`][`fs.lchownSync(path, uid, gid)`] было устаревшим. Отмена: в libuv
появились необходимые возможности.
...
Свойство [`require.extensions`][`require.extensions`] устарело.
...
Модуль [`punycode`][`punycode`] устарел. Используйте альтернативу из пользовательской экосистемы.
...
Класс `tls.CryptoStream` удалён. Используйте [`tls.TLSSocket`][`tls.TLSSocket`].
...
[`tls.TLSSocket`][`tls.TLSSocket`] instead.
...
The [`util.isArray()`][`util.isArray()`] API is deprecated. Please use `Array.isArray()`
instead.
...
[`Buffer.isBuffer()`][`Buffer.isBuffer()`] instead.
...
The [`util._extend()`][`util._extend()`] API is deprecated because it's an unmaintained
...
function for [`util.inspect()`][`util.inspect()`] is deprecated. Use [`util.inspect.custom`][`util.inspect.custom`]
...
OpenSSL when calling [`decipher.setAuthTag()`][`decipher.setAuthTag()`]. Beginning with Node.js
...
The [`crypto.fips`][`crypto.fips`] property is deprecated. Please use `crypto.setFips()`
...
[`setTimeout()`][`setTimeout()`] or [`setInterval()`][`setInterval()`] instead.
...
[`clearTimeout()`][`clearTimeout()`] or [`clearInterval()`][`clearInterval()`] instead.
...
Use [`asyncResource.runInAsyncScope()`][`asyncResource.runInAsyncScope()`] API instead which provides a much
...
`process.assert()` is deprecated. Please use the [`assert`][`assert`] module instead.
...
Тип: только документация (Documentation-only) (supports [`--pending-deprecation`][`--pending-deprecation`])

Using `process.binding()` in general should be avoided. The type checking
methods in particular can be replaced by using [`util.types`][`util.types`].
...
When assigning a non-string property to [`process.env`][`process.env`], the assigned value is
...
[`decipher.final()`][`decipher.final()`]. This API has been removed, and it is recommended to use
[`decipher.final()`][`decipher.final()`] instead.
...
[`crypto.pbkdf2()`][`crypto.pbkdf2()`] or [`crypto.scrypt()`][`crypto.scrypt()`] with random salts and to use
[`crypto.createCipheriv()`][`crypto.createCipheriv()`] and [`crypto.createDecipheriv()`][`crypto.createDecipheriv()`] to obtain the
[`Cipheriv`][`Cipheriv`] and [`Decipheriv`][`Decipheriv`] objects respectively.
...
Deprecated alias for [`zlib.bytesWritten`][`zlib.bytesWritten`]. This original name was chosen
...
[`https.get()`][`https.get()`], and [`tls.checkServerIdentity()`][`tls.checkServerIdentity()`] APIs because those were
...
[`script.createCachedData()`][`script.createCachedData()`] instead.
...
[`crypto.randomBytes()`][`crypto.randomBytes()`] and `crypto.pseudoRandomBytes()`. The latter is
deprecated along with the undocumented aliases `crypto.prng()` and
`crypto.rng()` in favor of [`crypto.randomBytes()`][`crypto.randomBytes()`] and might be removed in a
future release.
...
The [legacy URL API][legacy URL API] is deprecated. This includes [`url.format()`][`url.format()`],
[`url.parse()`][`url.parse()`], [`url.resolve()`][`url.resolve()`], and the [legacy `urlObject`][legacy `urlObject`]. Please
...
[`util.getSystemErrorName()`][`util.getSystemErrorName()`] instead.
...
Please use the publicly documented [`timeout.refresh()`][`timeout.refresh()`] instead.
If re-referencing the timeout is necessary, [`timeout.ref()`][`timeout.ref()`] can be used
...
Please use the publicly documented [`timeout.refresh()`][`timeout.refresh()`] instead.
If unreferencing the timeout is necessary, [`timeout.unref()`][`timeout.unref()`] can be used
with no performance impact since Node.js 10.
...
Use [`module.createRequire()`][`module.createRequire()`] instead.
...
Passing a callback to [`worker.terminate()`][`worker.terminate()`] is deprecated. Use the returned
...
Prefer [`response.socket`][`response.socket`] over [`response.connection`][`response.connection`] and
[`request.socket`][`request.socket`] over [`request.connection`][`request.connection`].
...
[`WriteStream.open()`][`WriteStream.open()`] and [`ReadStream.open()`][`ReadStream.open()`] are undocumented internal
APIs that do not make sense to use in userland. File streams should always be
opened through their corresponding factory methods [`fs.createWriteStream()`][`fs.createWriteStream()`]
and [`fs.createReadStream()`][`fs.createReadStream()`]) or by passing a file descriptor in options.
...
[`response.finished`][`response.finished`] indicates whether [`response.end()`][`response.end()`] has been
called, not whether `'finish'` has been emitted and the underlying data
is flushed.

Use [`response.writableFinished`][`response.writableFinished`] or [`response.writableEnded`][`response.writableEnded`]
...
Allowing a [`fs.FileHandle`][`fs.FileHandle`] object to be closed on garbage collection used
...
[`process.mainModule`][`process.mainModule`] is a CommonJS-only feature while `process` global
object is shared with non-CommonJS environment. Its use within ECMAScript
modules is unsupported.

It is deprecated in favor of [`require.main`][`require.main`], because it serves the same
...
Use [`request.destroy()`][`request.destroy()`] instead of [`request.abort()`][`request.abort()`].
...
[`socket.bufferSize`][`socket.bufferSize`] is just an alias for [`writable.writableLength`][`writable.writableLength`].
...
The [`crypto.Certificate()` constructor][`crypto.Certificate()` constructor] is deprecated. Use
[static methods of `crypto.Certificate()`][static methods of `crypto.Certificate()`] instead.
...
Prefer [`message.socket`][`message.socket`] over [`message.connection`][`message.connection`].
...
an explicit [`"exports"` or `"main"` entry][`"exports"` or `"main"` entry] with the exact file extension.
...
[`dns.lookup()`][`dns.lookup()`] and [`dnsPromises.lookup()`][`dnsPromises.lookup()`] throws an
...
Move to [Stream](stream.md#stream) API instead, as the [`http.ClientRequest`][`http.ClientRequest`],
[`http.ServerResponse`][`http.ServerResponse`], and [`http.IncomingMessage`][`http.IncomingMessage`] are all stream-based.
...
[`readable.readableEnded`][`readable.readableEnded`] value on [`http.IncomingMessage`][`http.IncomingMessage`] to get whether
...
Use [`buffer.subarray`][`buffer.subarray`] which does the same thing instead.
...
Use [`process.getActiveResourcesInfo()`][`process.getActiveResourcesInfo()`] to get a list of types of active
...
parameter in [`fs.write()`][`fs.write()`], [`fs.writeFile()`][`fs.writeFile()`], [`fs.appendFile()`][`fs.appendFile()`],
[`fs.writeFileSync()`][`fs.writeFileSync()`], and [`fs.appendFileSync()`][`fs.appendFileSync()`] is deprecated.
...
[`process.exit()`][`process.exit()`] and as value to assign to [`process.exitCode`][`process.exitCode`].
...
Set the flag [`--force-node-api-uncaught-exceptions-policy`][`--force-node-api-uncaught-exceptions-policy`] to force Node.js
to emit an [`'uncaughtException'`][`'uncaughtException'`] event if the exception is not handled in
Node-API callbacks.
...
[`url.parse()`][`url.parse()`] behavior is not standardized and prone to errors that
have security implications. Use the [WHATWG URL API][WHATWG URL API] instead. CVEs are not
issued for `url.parse()` vulnerabilities.

Calling [`url.format(urlString)`][`url.format(urlString)`] or [`url.resolve()`][`url.resolve()`] invokes `url.parse()`
...
[`url.parse()`][`url.parse()`] used to accept URLs with ports that are not numbers. This
...
In a future version of Node.js, [`message.headers`][`message.headers`],
[`message.headersDistinct`][`message.headersDistinct`], [`message.trailers`][`message.trailers`], and
[`message.trailersDistinct`][`message.trailersDistinct`] will be read-only.
...
Calling [`util.promisify`][`util.promisify`] on a function that returns a `Promise` will ignore
...
The [`util.toUSVString()`][`util.toUSVString()`] API is deprecated. Please use
[`String.prototype.toWellFormed`][`String.prototype.toWellFormed`] instead.
...
release lines. Please use [`dirent.parentPath`][`dirent.parentPath`] instead.
...
Please use the [`crypto.createHash()`][`crypto.createHash()`] method to create Hash instances.
...
deprecated due to being internals, not intended for public use.
Please use the [`crypto.createHmac()`][`crypto.createHmac()`] method to create Hmac instances.
...
For ciphers in GCM mode, the [`decipher.setAuthTag()`][`decipher.setAuthTag()`] function used to accept
...
[`crypto.createDecipheriv()`][`crypto.createDecipheriv()`] function to the appropriate length.
...
`https.request()`, [`tls.createSecureContext()`][`tls.createSecureContext()`], and [`tls.createServer()`][`tls.createServer()`];
the `privateKeyEngine` and `privateKeyIdentifier` for [`tls.createSecureContext()`][`tls.createSecureContext()`];
and [`crypto.setEngine()`][`crypto.setEngine()`] all depend on this functionality from OpenSSL.
...
When an `args` array is passed to [`child_process.execFile`][`child_process.execFile`] or [`child_process.spawn`][`child_process.spawn`] with the option
...
To make [`child_process.execFile`][`child_process.execFile`] or [`child_process.spawn`][`child_process.spawn`] invoke the
...
To make [`child_process.exec`][`child_process.exec`] invoke the default shell, either omit the
`shell` option, or set it to a nullish value. If the intention is not to invoke
a shell, use [`child_process.execFile`][`child_process.execFile`] instead.
...
The [`util.types.isNativeError`][`util.types.isNativeError`] API is deprecated. Please use [`Error.isError`][`Error.isError`] instead.
...
Allowing a [`fs.Dir`][`fs.Dir`] object to be closed on garbage collection is
...
Passing the `type` option to [`Duplex.toWeb()`][`Duplex.toWeb()`] is deprecated. To specify the
...
[`http2.createServer()`][`http2.createServer()`] and [`http2.createSecureServer()`][`http2.createSecureServer()`] are
...
Passing a [`CryptoKey`][`CryptoKey`] to `node:crypto` functions is deprecated and
will throw an error in a future version. This includes
[`crypto.createPublicKey()`][`crypto.createPublicKey()`], [`crypto.createPrivateKey()`][`crypto.createPrivateKey()`],
[`crypto.sign()`][`crypto.sign()`], [`crypto.verify()`][`crypto.verify()`],
[`crypto.publicEncrypt()`][`crypto.publicEncrypt()`], [`crypto.publicDecrypt()`][`crypto.publicDecrypt()`],
[`crypto.privateEncrypt()`][`crypto.privateEncrypt()`], [`crypto.privateDecrypt()`][`crypto.privateDecrypt()`],
[`Sign.prototype.sign()`][`Sign.prototype.sign()`], [`Verify.prototype.verify()`][`Verify.prototype.verify()`],
[`crypto.createHmac()`][`crypto.createHmac()`], [`crypto.createCipheriv()`][`crypto.createCipheriv()`],
[`crypto.createDecipheriv()`][`crypto.createDecipheriv()`], [`crypto.encapsulate()`][`crypto.encapsulate()`], and
[`crypto.decapsulate()`][`crypto.decapsulate()`].
...
Passing a non-extractable [`CryptoKey`][`CryptoKey`] to [`KeyObject.from()`][`KeyObject.from()`] is
...
[`module.register()`][`module.register()`] is deprecated. Use [`module.registerHooks()`][`module.registerHooks()`]
instead.
