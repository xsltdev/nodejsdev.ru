...
[`--pending-deprecation`][] (или переменной окружения
...
С флагом [`--throw-deprecation`][] устаревание времени выполнения
приводит к выбросу ошибки. С
[`--pending-deprecation`][] предупреждения также выдаются для
...
* [`Buffer.allocUnsafeSlow(size)`][]: создаёт `Buffer` с _неинициализированной_
  памятью.
* [`Buffer.from(array)`][]: создаёт `Buffer`, копируя `array`.
* [`Buffer.from(arrayBuffer[, byteOffset[, length]])`][from_arraybuffer] -
  создаёт `Buffer`, оборачивающий указанный `arrayBuffer`.
* [`Buffer.from(buffer)`][]: создаёт `Buffer`, копируя `buffer`.
...
В методах `spawn()`, `fork()` и `exec()` модуля [`child_process`][]
...
[`worker.exitedAfterDisconnect`][]: старое имя плохо отражало семантику и было
...
Использование API [`crypto.pbkdf2()`][] без указания `digest` было объявлено устаревшим
...
[`tls.createSecureContext()`][] вместо него.
...
Класс `crypto.Credentials` удалён. Используйте [`tls.SecureContext`][]
вместо него.
...
Устаревший `String`-интерфейс [`fs.read()`][] объявлен устаревшим. Вместо него
...
Устаревший `String`-интерфейс [`fs.readSync()`][] объявлен устаревшим. Вместо него
используйте API `Buffer`, как указано в документации.
...
параметр командной строки [`--unhandled-rejections`][].
...
удалено. Используйте метод [`Server.getConnections()`][] вместо него.
...
[`Server.listen({fd: <number>})`][] вместо него.
...
Используйте [`os.tmpdir()`][] вместо него.
...
[`os.networkInterfaces()`][] вместо него.
...
API [`events.listenerCount(emitter, eventName)`][] было устаревшим, так как
дублировало [`emitter.listenerCount(eventName)`][]. Устаревание отменено: функция
...
API [`fs.exists(path, callback)`][] устарело. Используйте
[`fs.stat()`][] или [`fs.access()`][].
...
API [`fs.lchown(path, uid, gid, callback)`][] было устаревшим. Отмена: в libuv
...
API [`fs.lchownSync(path, uid, gid)`][] было устаревшим. Отмена: в libuv
появились необходимые возможности.
...
Move to [Stream](stream.md#stream) API instead, as the [`http.ClientRequest`][],
