...
(Maximum Transmission Unit) и размера поля `Payload Length`.
...
* Поле `Payload Length` имеет ширину 16 бит, поэтому обычная полезная нагрузка
  ...
  это обычно для loopback, но такие длинные датаграммы непрактичны для большинства хостов и сетей.
...
* `MTU` — максимальный размер, который поддерживает технология канального уровня
  ...
  рекомендуемый `MTU` для IPv4 — 576 (часто для dial-up), целиком или фрагментами.

  Для IPv6 минимальный `MTU` — 1280 октетов. Обязательный минимальный размер буфера
  сборки фрагментов — 1500 октетов. 68 октетов мало: у большинства сетей, например Ethernet,
  `MTU` не меньше 1500.
...
Вызов на сокете, не готовом к отправке или уже закрытом, может выбросить _Not running_ [`Error`](errors.md#class-error).

Если `multicastInterface` нельзя разобрать как IP, выбрасывается _EINVAL_ [`System Error`](errors.md#class-systemerror).

В IPv4 при корректном, но не совпадающем с интерфейсом адресе или несовпадении семейства —
[`System Error`](errors.md#class-systemerror), например `EADDRNOTAVAIL` или `EPROTONOSUP`.
