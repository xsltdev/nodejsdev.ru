...
Несмотря на название [системы доменных имён (DNS)][], для поиска не всегда используется протокол DNS. [`dns.lookup()`][] опирается на средства ОС для разрешения имён. Сетевой обмен может и не понадобиться. Чтобы вести себя при разрешении имён так же, как другие приложения в системе, используйте [`dns.lookup()`][].
...
конфигурационных файлов у них другой, чем у [`dns.lookup()`][] (например, не
...
Возвращает массив строк IP-адресов в формате [RFC 5952][],
...
* `options` [<integer>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  ...
  * `hints` [<number>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Одна или несколько [supported `getaddrinfo` flags][].
  ...
  * `order` [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) При `verbatim` адреса возвращаются в том порядке, как выдал резолвер.
    ...
    **По умолчанию:** `verbatim` (порядок не меняется).
    Значение по умолчанию задаётся через [`dns.setDefaultResultOrder()`][] или
    [`--dns-result-order`][].
  * `verbatim` [<boolean>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, IPv4 и IPv6 приходят в порядке, который вернул DNS-резолвер.
    ...
    **По умолчанию:** `true` (порядок не меняется). Значение по умолчанию настраивается через
    [`dns.setDefaultResultOrder()`][] или [`--dns-result-order`][].
...
При ошибке `err` — объект [`Error`][], поле `err.code` — код ошибки.
...
Если метод вызывается через [`util.promisify()`][], и `all` не равен `true`,
...
В качестве подсказок для [`dns.lookup()`][] можно передать такие флаги:
...
Если метод обёрнут в [`util.promisify()`][], возвращается `Promise` с объектом
`{ hostname, service }`.
...
| --------- | ------------------------------- | -------------- | ------------------------ |
| `'A'`     | IPv4-адреса (по умолчанию)      | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dns.resolve4()`][]     |
| `'AAAA'`  | IPv6-адреса                     | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dns.resolve6()`][]     |
| `'ANY'`   | любые записи                    | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveAny()`][]   |
| `'CAA'`   | записи авторизации ЦС           | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveCaa()`][]   |
| `'CNAME'` | канонические имена              | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dns.resolveCname()`][] |
| `'MX'`    | почтовый обмен                  | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveMx()`][]    |
| `'NAPTR'` | записи NAPTR                    | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveNaptr()`][] |
| `'NS'`    | серверы имён                    | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dns.resolveNs()`][]    |
| `'PTR'`   | указатели                       | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dns.resolvePtr()`][]   |
| `'SOA'`   | начало зоны                     | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveSoa()`][]   |
| `'SRV'`   | службы                          | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveSrv()`][]   |
| `'TLSA'`  | привязки сертификатов           | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dns.resolveTlsa()`][]  |
| `'TXT'`   | текстовые записи                | [<string[]>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)    | [`dns.resolveTxt()`][]   |

При ошибке `err` — объект [`Error`][], поле `err.code` — один из
[DNS error codes][].
...
| `'TXT'`   | У записи есть массив `entries`, см. [`dns.resolveTxt()`][], напр. `{ entries: ['...'], type: 'TXT' }`                                            |
...
надёжнее вызывать отдельные методы вроде [`dns.resolve4()`][],
[`dns.resolveMx()`][] и т. д. Подробнее — [RFC 8482][].
...
Задаёт значение `order` по умолчанию для [`dns.lookup()`][] и
[`dnsPromises.lookup()`][]:
...
По умолчанию используется `verbatim`; [`dns.setDefaultResultOrder()`][] имеет
более высокий приоритет, чем [`--dns-result-order`][]. В [worker threads][]
вызов [`dns.setDefaultResultOrder()`][] из основного потока не меняет порядок DNS
в воркерах.
...
Возвращает значение `order` по умолчанию для [`dns.lookup()`][] и
[`dnsPromises.lookup()`][]:
...
* `servers` [<string[]>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) массив адресов в формате [RFC 5952][]

Задаёт IP-адреса и порты DNS-серверов для разрешения имён. Аргумент `servers` —
массив строк в формате [RFC 5952][]. Порт по умолчанию IANA для DNS (53) можно не указывать.
...
[`dns.setServers()`][] влияет только на [`dns.resolve()`][],
`dns.resolve*()` и [`dns.reverse()`][], но **не** на [`dns.lookup()`][].
...
Возвращает массив строк IP в формате [RFC 5952][],
...
* `options` [<integer>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  ...
  * `order` [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) При `verbatim` порядок адресов как у DNS-резолвера. При `ipv4first` —
    ...
    **По умолчанию:** `verbatim` (порядок не меняется).
    Значение по умолчанию задаётся через [`dns.setDefaultResultOrder()`][] или
    [`--dns-result-order`][]. В новом коде используйте `{ order: 'verbatim' }`.
  * `verbatim` [<boolean>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, порядок IPv4/IPv6 как у резолвера. Если `false`,
    ...
    **По умолчанию:** сейчас `false` (порядок меняется), в будущем может измениться.
    Настраивается через [`dns.setDefaultResultOrder()`][] или
    [`--dns-result-order`][].
...
При ошибке промис отклоняется с [`Error`][], поле `err.code` — код ошибки.
`err.code` бывает `'ENOTFOUND'` не только при отсутствии имени, но и при других сбоях,
например нехватке дескрипторов.

[`dnsPromises.lookup()`][] не обязан использовать протокол DNS; используются средства ОС.
...
| `rrtype`  | Содержимое `records`            | Тип результата | Краткий метод                    |
| --------- | ------------------------------- | -------------- | -------------------------------- |
| `'A'`     | IPv4 (по умолчанию)             | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dnsPromises.resolve4()`][]     |
| `'AAAA'`  | IPv6                            | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dnsPromises.resolve6()`][]     |
| `'ANY'`   | любые записи                    | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveAny()`][]   |
| `'CAA'`   | авторизация ЦС                  | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveCaa()`][]   |
| `'CNAME'` | канонические имена              | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dnsPromises.resolveCname()`][] |
| `'MX'`    | почтовый обмен                  | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveMx()`][]    |
| `'NAPTR'` | NAPTR                           | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveNaptr()`][] |
| `'NS'`    | серверы имён                    | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dnsPromises.resolveNs()`][]    |
| `'PTR'`   | указатели                       | [<string>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)       | [`dnsPromises.resolvePtr()`][]   |
| `'SOA'`   | начало зоны                     | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveSoa()`][]   |
| `'SRV'`   | службы                          | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveSrv()`][]   |
| `'TLSA'`  | привязки сертификатов           | [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)       | [`dnsPromises.resolveTlsa()`][]  |
| `'TXT'`   | текстовые записи                | [<string[]>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)    | [`dnsPromises.resolveTxt()`][]   |

При ошибке промис отклоняется с [`Error`][], поле `err.code` — один из [DNS error codes][].
...
| `'TXT'`   | массив `entries`, см. [`dnsPromises.resolveTxt()`][], напр. `{ entries: ['...'], type: 'TXT' }`                                                            |
...
По умолчанию `verbatim`; [`dnsPromises.setDefaultResultOrder()`][] важнее
[`--dns-result-order`][]. В [worker threads][] вызов из основного потока не меняет порядок DNS в воркерах.
...
Задаёт IP и порты DNS-серверов. Аргумент `servers` — массив строк [RFC 5952][]; порт 53 можно не указывать.
...
И [`dns.lookup()`][], и функции `dns.resolve*()` / `dns.reverse()` сопоставляют сетевое имя и адрес (или наоборот), но делают это по-разному. Это влияет на поведение приложений Node.js.

### `dns.lookup()`

Внутри [`dns.lookup()`][] использует те же механизмы ОС, что и большинство программ: обычно имя разрешается так же, как в команде `ping`. В POSIX-системах поведение можно менять через nsswitch.conf(5) и resolv.conf(5), и это затронет все программы на машине.

С точки зрения JavaScript вызов асинхронный, но под капотом выполняется синхронный getaddrinfo(3) в пуле потоков libuv — см. [`UV_THREADPOOL_SIZE`][].

Многие сетевые API внутри вызывают `dns.lookup()`. Если это мешает, заранее разрешите имя через `dns.resolve()` и передавайте адрес. Часть API ([`socket.connect()`][], [`dgram.createSocket()`][] и др.) позволяет подменить резолвер по умолчанию (`dns.lookup()`).

### `dns.resolve()`, `dns.resolve*()` и `dns.reverse()`

Эти функции устроены иначе, чем [`dns.lookup()`][]: не используют getaddrinfo(3) и **всегда** выполняют DNS-запрос по сети. Обмен асинхронный и не задействует пул потоков libuv.

Поэтому они не создают такой же конкуренции за пул, как [`dns.lookup()`][].
