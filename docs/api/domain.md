...
Из-за того, как в JavaScript работает [`throw`][], почти нельзя безопасно
...
(см. [`domain.exit()`][]). Вызов `enter()` ограничивает начало цепочки асинхронных вызовов и операций ввода-вывода,
...
Почти то же, что [`domain.bind(callback)`][], но помимо перехвата выброшенных ошибок перехватывает
объекты [`Error`][], переданные первым аргументом в функцию.
