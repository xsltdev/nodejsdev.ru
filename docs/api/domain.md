...
Из-за того, как в JavaScript работает [`throw`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/throw), почти нельзя безопасно «продолжить с того же места», не допустив утечек ссылок или хрупкого неопределённого состояния.
...
Метод `enter()` — внутренняя часть реализации `run()`, `bind()` и `intercept()`: задаёт активный домен. Устанавливает `domain.active` и `process.domain` на этот домен и помещает домен в стек доменов (см. [`domain.exit()`](#domainexit)). Вызов `enter()` ограничивает начало цепочки асинхронных вызовов и операций ввода-вывода, привязанных к домену.
...
Почти то же, что [`domain.bind(callback)`](#domainbindcallback), но помимо перехвата выброшенных ошибок перехватывает объекты [`Error`](errors.md#class-error), переданные первым аргументом в функцию.
