...
[`try…catch`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch), которую предоставляет язык JavaScript.
...
или не используется устаревший модуль [`node:domain`](domain.md).
...
отражает место вызова `new Error()`. Трассировки зависят от [API трассировки стека V8][API трассировки стека V8].
...
в разделе [кодов ошибок Node.js][кодов ошибок Node.js].
...
см. на странице [страницы руководства `errno`(3)][страницы руководства `errno`(3)].

* `EACCES` (доступ запрещён): попытка доступа к файлу способом, запрещённым правами.

* `EADDRINUSE` (адрес уже используется): не удалось привязать сервер ([`net`][], [`http`][]
  или [`https`][]) к локальному адресу — его уже занимает другой сервер на этой машине.

* `ECONNREFUSED` (в соединении отказано): соединение отклонено — целевая машина активно
  отказала. Обычно сервис на удалённом хосте не слушает порт.

* `ECONNRESET` (соединение сброшено удалённой стороной): соединение разорвано удалённой стороной,
  часто из‑за таймаута или перезагрузки. Типично в модулях [`http`][] и [`net`][].

* `EEXIST` (файл уже существует): целью операции был существующий файл, тогда как требовалось,
  чтобы файла не было.

* `EISDIR` (это каталог): ожидался файл, а по указанному пути — каталог.

* `EMFILE` (слишком много открытых файлов в системе): достигнут предел [дескрипторов файлов][file descriptors]
...
* `ENOENT` (нет такого файла или каталога): часто от операций [`fs`][] — компонент пути
  не существует; по указанному пути не найден ни файл, ни каталог.

* `ENOTDIR` (не каталог): компонент пути существует, но это не каталог, как ожидалось.
  Часто от [`fs.readdir`][].

* `ENOTEMPTY` (каталог не пуст): каталог не пуст, а операция требует пустого каталога;
...
* `EPIPE` (разорванный канал): запись в pipe, сокет или FIFO, когда читателя нет. Часто на уровнях
  [`net`][] и [`http`][] — удалённая сторона потока закрыла соединение.

* `ETIMEDOUT` (время операции истекло): таймаут connect/send — удалённая сторона не ответила
...
Специальный тип ошибки, возникающий всякий раз, когда Node.js пытается получить доступ к ресурсу, ограниченному [моделью разрешений][Permission Model].
...
Была предпринята попытка использовать операции KEM, хотя Node.js был собран
без OpenSSL с поддержкой KEM.
...
Node.js был собран без поддержки `scrypt`. Это невозможно для официальных
бинарных релизов, но может случиться в пользовательских сборках, включая
сборки дистрибутивов.
...
Для разбора в [конструктор `URLPattern`](url.md#new-urlpatternstring-baseurl-options)
по [WHATWG URL API][WHATWG URL API] был передан недопустимый URLPattern.
...
При попытке `require()` [ES-модуля][ES Module] оказалось, что модуль асинхронный, то есть
содержит `await` верхнего уровня.

Чтобы увидеть, где находится `await` верхнего уровня, используйте
...
При попытке `require()` [ES-модуля][ES Module] участвует в немедленном цикле граница
CommonJS↔ESM.
...
* голый спецификатор, который не является встроенным модулем, из модуля, чья URL-схема
  не равна `file`;
* [относительный URL][relative URL] из модуля, чья URL-схема не является [специальной схемой][special scheme].
...
Была использована возможность, требующая WebAssembly, но WebAssembly не поддерживается
или отключён в текущем окружении (например, при запуске с `--jitless`).
...
Было получено слишком много данных для расширений чанка. Чтобы защититься от
вредоносных или неверно настроенных клиентов, при получении более 16 КиБ данных
генерируется `Error` с этим кодом.
...
Загрузчик модулей CommonJS не смог разрешить файл модуля при попытке выполнить
операцию [`require()`][] или при загрузке точки входа программы.
...
В [`crypto.scrypt()`][] или [`crypto.scryptSync()`][] была передана
несовместимая комбинация параметров. В новых версиях Node.js вместо этого
используется код ошибки [`ERR_INCOMPATIBLE_OPTION_PAIR`][], что согласуется с
другими API.
...
В методы [`fs.symlink()`][] или [`fs.symlinkSync()`][] был передан недопустимый тип симлинка.
...
При использовании API измерения производительности (`perf_hooks`) метка производительности
оказалась недопустимой.
...
[`ERR_MISSING_MESSAGE_PORT_IN_TRANSFER_LIST`][]. Однако набор переносимых
типов объектов был расширен и теперь охватывает больше типов, чем только
`MessagePort`.
...
В основном потоке значения удаляются из очереди, связанной с потокобезопасной
функцией, в цикле ожидания. Эта ошибка означает, что при попытке запустить этот
цикл произошёл сбой.
...
Когда в очереди больше не остаётся элементов, цикл ожидания должен быть
приостановлен. Эта ошибка означает, что цикл ожидания не смог остановиться.

### `ERR_NO_LONGER_SUPPORTED` {#ERR_NO_LONGER_SUPPORTED}

API Node.js был вызван неподдерживаемым способом, например
...
Используется как общий код для обозначения того, что операция привела к
состоянию нехватки памяти.
...
[кодов ошибок Node.js]: #nodejs-error-codes
[Permission Model]: permissions.md#permission-model
[RFC 7230 Section 3]: https://tools.ietf.org/html/rfc7230#section-3
[SQLite]: sqlite.md
[Subresource Integrity specification]: https://www.w3.org/TR/SRI/#the-integrity-attribute
[API трассировки стека V8]: https://v8.dev/docs/stack-trace-api
...
[страницы руководства `errno`(3)]: https://man7.org/linux/man-pages/man3/errno.3.html
...
[`http`]: http.md
[`https`]: https.md
[`обработке ошибок libuv`]: https://docs.libuv.org/en/v1.x/errors.html
