...
для повторного использования. Модули задаются с помощью различных операторов [`import`][`import`] и
[`export`][`export`].
...
файла `.mjs`, поле `package.json` [`"type"`][`"type"`] со значением `"module"`
или флаг [`--input-type`][`--input-type`] со значением `"module"`. Это явные признаки того, что код
должен выполняться как ES-модуль.

Наоборот, авторы могут явно указать Node.js интерпретировать JavaScript как
CommonJS через расширение `.cjs`, поле `package.json` [`"type"`][`"type"`] со значением
`"commonjs"` или флаг [`--input-type`][`--input-type`] со значением `"commonjs"`.
...
* _Голые спецификаторы_, например `'some-package'` или `'some-package/shuffle'`. Они
  ...
  _Указывать расширение файла нужно только для пакетов без поля [`"exports"`][`"exports"`]._
...
если только в [`package.json`][`package.json`] пакета нет поля [`"exports"`][`"exports"`] — тогда файлы внутри
пакета доступны только по путям, заданным в [`"exports"`][`"exports"`].
...
[`data:` URLs][`data:` URLs] поддерживаются для импорта со следующими MIME-типами:
...
Операторы `import` допустимы только в ES-модулях, но динамические выражения [`import()`][`import()`]
...
Подробнее см. [Loading ECMAScript modules using `require()`][Loading ECMAScript modules using `require()`].
...
поддержке взаимодействия [`require(esm)`][`require(esm)`].
...
[`import.meta.filename`][`import.meta.filename`] и [`import.meta.dirname`][`import.meta.dirname`].

#### Нет загрузки аддонов

Импорты ES-модулей [аддоны][Addons] сейчас не поддерживают.

Их можно загрузить через [`module.createRequire()`][`module.createRequire()`] или
[`process.dlopen`][`process.dlopen`].

#### Нет `require.main`

Вместо `require.main === module` есть API [`import.meta.main`][`import.meta.main`].
