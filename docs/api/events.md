...
Например, объект [`net.Server`](net.md#class-netserver) испускает событие при каждом подключении пира;
[`fs.ReadStream`](fs.md#class-fsreadstream) — когда файл открыт; [stream][stream] — когда данные доступны для чтения.
...
модуль [`domain`](domain.md). (Однако модуль `node:domain` устарел.)
...
Он асинхронно передаёт исключение в метод [`Symbol.for('nodejs.rejection')`](#emittersymbolfornodejsrejectionerr-eventname-args),
если он есть, иначе — в обработчик события [`'error'`](#error-events).
...
заданное через [`emitter.setMaxListeners(n)`](#emittersetmaxlistenersn), либо по умолчанию
...
испускании события, если у эмиттера включён [`captureRejections`](#capture-rejections-of-promises).
...
[`emitter.setMaxListeners(n)`](#emittersetmaxlistenersn). Чтобы изменить значение по умолчанию для всех
...
[`emitter.setMaxListeners(n)`](#emittersetmaxlistenersn) по-прежнему имеет приоритет над
`events.defaultMaxListeners`.
...
`AbortSignal` по-прежнему можно задать порог через [`emitter.setMaxListeners(n)`](#emittersetmaxlistenersn),
...
Флаг командной строки [`--trace-warnings`](cli.md#--trace-warnings) включает вывод трассировки стека для
таких предупреждений.

Испущенное предупреждение можно разобрать в [`process.on('warning')`](process.md#event-warning): у него
...
утечкам: другой код с тем же сигналом может вызвать [`e.stopImmediatePropagation()`](#eventstopimmediatepropagation).
