...
`` mjs
import { access, constants } из 'node:fs/promises';
...
mjs
import { copyFile, constants } из 'node:fs/promises';
...
``mjs
import { access, constants } из 'node:fs';
...
access(file, constants.F_OK, (err) => {
  console.log(`${файл} ${err ? 'не существует' : 'существует'}`);
});
...
appendFile('message.txt', 'данные для добавления', (err) => {
  if (err) throw err;
...
open('message.txt', 'a', (err, fd) => {
  ...
  try {
    appendFile(fd, 'данные для добавления', 'utf8', (err) => {
      closeFd(fd);
      if (err) throw err;
    });
  } catch (err) {
...
Caveats: on Windows only the write permission can be changed, and the distinction among the permissions of group, owner, or
...
mjs
import { copyFile, constants } из 'node:fs';
...
> Стабильность: 0 - Исправлено: Вместо этого используйте [`fs.stat()`](#fsstatpath-options-callback) или [`fs.access()`](#fsaccesspath-mode-callback).
...
`` mjs
import { open, close, exists } из 'node:fs';
...
Статистика {
  ...
  время рожденияМс: 1561174126937.2893,
...
Статистика {
  ...
  время рожденияМс: 1561174007710.7478,
...
Обратный вызов получает аргументы `(err, written, string)`, где `written` указывает, сколько *байт* переданной строки требуется записать. Записанные байты не обязательно совпадают с записанными символами строки. См. [`Buffer.byteLength`](buffer.md#static-method-bufferbytelengthstring-encoding).
...
`` mjs
import { accessSync, constants } из 'node:fs';
...
По умолчанию все объекты {fs.FSWatcher} являются "ref'ed", поэтому обычно нет необходимости вызывать `watcher.ref()`, если только `watcher.unref()` не был вызван ранее.
...
Stats {
  ...
  время рожденияМс: 1318289051000.1,
  atime: Mon, 10 Oct 2011 23:24:11 GMT,
  mtime: Mon, 10 Oct 2011 23:24:11 GMT,
  ctime: Mon, 10 Oct 2011 23:24:11 GMT,
  время рождения: Mon, 10 Oct 2011 23:24:11 GMT }
```
...
BigIntStats {
  ...
  время рожденияМс: 1318289051000n,
  времяНс: 1318289051000000000n,
  mtimeNs: 1318289051000000000n,
  ctimeNs: 1318289051000000000n,
  время рожденияNs: 1318289051000000000n,
  atime: Mon, 10 Oct 2011 23:24:11 GMT,
  mtime: Mon, 10 Oct 2011 23:24:11 GMT,
  ctime: Mon, 10 Oct 2011 23:24:11 GMT,
  время рождения: Mon, 10 Oct 2011 23:24:11 GMT }
...
``mjs
import { open, constants } из 'node:fs';
...
Flag indicating that the file can be executed by the calling process. This has no effect on Windows (will behave like <code>fs.constants.F\_OK</code>).
...
<code>COPYFILE\_FICLONE\_FORCE</code>
...
Flag indicating that opening a file should fail if the <code>O\_CREAT</code> flag is set and the file already exists.
