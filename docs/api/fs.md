...
Объект [FileHandle](#class-filehandle) — обёртка над числовым дескриптором файла.

Экземпляры [FileHandle](#class-filehandle) создаются методом `fsPromises.open()`.

Все объекты [FileHandle](#class-filehandle) являются [EventEmitter](events.md#class-eventemitter).

Если [FileHandle](#class-filehandle) не закрыть через `filehandle.close()`, среда попытается автоматически закрыть дескриптор и выдать предупреждение процессу, чтобы уменьшить риск утечек памяти. На такое поведение полагаться не следует: оно может сработать ненадёжно, и файл может остаться открытым. Всегда явно закрывайте [FileHandle](#class-filehandle). В будущем Node.js может изменить эту логику.

#### Событие: `'close'`

Событие `'close'` генерируется, когда [FileHandle](#class-filehandle) закрыт и больше использоваться не может.
...
-   Возвращает: [`<fs.WriteStream>`](fs.md#class-fswritestream)
...
-   Тип: [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) числовой дескриптор файла, которым управляет объект [FileHandle](#class-filehandle).
...
-   `...transforms` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) необязательные преобразования через [`stream/iter pullSync()`](stream_iter.md#pullsyncsource-transforms-options).
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `autoClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) закрыть дескриптор файла по окончании потока. **По умолчанию:** `false`.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `autoClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, [FileHandle](#class-filehandle) будет закрыт при закрытии потока. **По умолчанию:** `false`
...
[`FileHandle`](#class-filehandle) должен поддерживать чтение.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Должны ли числовые значения в возвращаемом объекте [fs.Stats](fs.md#class-fsstats) быть `bigint`. **По умолчанию:** `false`.
    -   `signal` [`<AbortSignal>`](globals.md#abortsignal) AbortSignal для отмены операции. **По умолчанию:** `undefined`.
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) При успешном выполнении исполняется объектом [fs.Stats](fs.md#class-fsstats) для файла.
...
Изменяет временные метки файловой системы для объекта, на который ссылается [FileHandle](#class-filehandle), и при успешном выполнении исполняет промис без аргументов.
...
[`FileHandle`](#class-filehandle) должен поддерживать запись.
...
Методы `writeSync()` и `writevSync()` включают быстрый try-sync путь, используемый [`stream/iter pipeTo()`](stream_iter.md#pipetosource-transforms-writer-options). Когда размер чанка reader совпадает с `chunkSize` у writer, все записи в конвейере `pipeTo()` завершаются синхронно без накладных расходов на промисы.
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](#class-filehandle) Имя файла или [FileHandle](#class-filehandle)
...
`path` может быть задан как [FileHandle](#class-filehandle), открытый для добавления (через `fsPromises.open()`).
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `cwd` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<URL>`](url.md#the-whatwg-url-api) Текущий рабочий каталог. **По умолчанию:** `process.cwd()`
    -   `exclude` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) | [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Функция для исключения файлов и каталогов из выборки либо список glob-шаблонов для исключения. Если передана функция, верните `true`, чтобы исключить элемент, и `false`, чтобы его включить. **По умолчанию:** `undefined`. Если передан массив строк, каждая строка должна быть glob-шаблоном, задающим исключаемые пути. Примечание: шаблоны с отрицанием (например, `'!foo.js'`) не поддерживаются.
    -   `withFileTypes` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, glob возвращает пути в виде объектов `Dirent`; если `false` — в виде строк. **По умолчанию:** `false`.
-   Возвращает: [`<AsyncIterator>`](https://tc39.es/ecma262/#sec-asynciterator-interface) Асинхронный итератор, последовательно отдающий пути к файлам, соответствующим шаблону.
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) должны ли числовые значения в возвращаемом объекте [fs.Stats](fs.md#class-fsstats) иметь тип `bigint`. **По умолчанию:** `false`.
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) при успехе выполняется объектом [fs.Stats](fs.md#class-fsstats) для указанной символической ссылки `path`.
...
Подробнее см. документацию [`fsPromises.mkdtemp()`](#fspromisesmkdtempprefix-options).

Необязательный аргумент `options` может быть строкой, задающей кодировку, либо объектом со свойством `encoding`, задающим используемую кодировку символов.

### `fsPromises.open(path, flags[, mode])`

-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) -- `flags` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) см. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'r'`.
...
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) при успехе выполняется объектом [FileHandle](#class-filehandle).

Открывает [FileHandle](#class-filehandle).

Подробнее см. документацию POSIX `open(2)`.

Некоторые символы (`< > : " / \ | ? *`) в Windows зарезервированы, см. [Имена файлов, путей и пространств имён](https://learn.microsoft.com/en-us/windows/win32/fileio/naming-a-file). В NTFS, если имя файла содержит двоеточие, Node.js откроет поток файловой системы, как описано на [этой странице Microsoft Learn](https://learn.microsoft.com/en-us/windows/win32/fileio/using-streams).
...
Если `options.withFileTypes` установлено в `true`, возвращаемый массив будет содержать объекты [fs.Dirent](fs.md#class-fsdirent).
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](#class-filehandle) filename or `FileHandle`
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | null **По умолчанию:** `null`
...
Любой указанный [FileHandle](#class-filehandle) должен поддерживать чтение.
...
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) при успехе выполняется объектом [fs.Stats](fs.md#class-fsstats) для указанного `path`.

### `fsPromises.statfs(path[, options])`

-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) должны ли числовые значения в возвращаемом объекте [fs.StatFs](fs.md#class-fsstatfs) иметь тип `bigint`. **По умолчанию:** `false`.
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) при успехе выполняется объектом [fs.StatFs](fs.md#class-fsstatfs) для указанного `path`.
...
-   `file` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](#class-filehandle) filename or `FileHandle`
-   `data` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) | [`<AsyncIterable>`](https://tc39.es/ecma262/#sec-asynciterable-interface) | [`<Iterable>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Iteration_protocols#The_iterable_protocol) | [`<Stream>`](stream.md#stream)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | null **По умолчанию:** `'utf8'`
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
...
Любой указанный [FileHandle](#class-filehandle) должен поддерживать запись.
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) имя файла или файловый дескриптор
-   `data` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | null **По умолчанию:** `'utf8'`
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `flag` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) См. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'a'`.
    -   `flush` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, перед закрытием дескриптора файла выполняется сброс буфера на диск. **По умолчанию:** `false`.
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api)
-   `options` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `flags` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) См. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'r'`.
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) **По умолчанию:** `null`
    -   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | [`<FileHandle>`](#class-filehandle) **По умолчанию:** `null`
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `autoClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `true`
    -   `emitClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `true`
    -   `start` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type)
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api)
-   `options` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `flags` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) См. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'w'`.
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) **По умолчанию:** `'utf8'`
    -   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | [`<FileHandle>`](#class-filehandle) **По умолчанию:** `null`
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `autoClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `true`
...
Как и [fs.ReadStream](fs.md#class-fsreadstream), если указан `fd`, [fs.WriteStream](fs.md#class-fswritestream) игнорирует аргумент `path` и использует переданный файловый дескриптор. Это означает, что событие `'open'` сгенерировано не будет. `fd` должен быть блокирующим; неблокирующие `fd` следует передавать в [net.Socket](net.md#class-netsocket).
...
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
    -   `stats` [`<fs.Stats>`](fs.md#class-fsstats)

Вызывает callback с объектом [fs.Stats](fs.md#class-fsstats) для файлового дескриптора.
...
Изменяет временные метки файловой системы для объекта, на который ссылается переданный файловый дескриптор. См. [`fs.utimes()`](#fsutimespath-atime-mtime-callback).
...
Получает [fs.Stats](fs.md#class-fsstats) для символической ссылки, на которую указывает путь. Callback получает два аргумента `(err, stats)`, где `stats` — объект [fs.Stats](fs.md#class-fsstats). `lstat()` идентичен `stat()`, за исключением того, что если `path` является символической ссылкой, статистика берётся для самой ссылки, а не для файла, на который она указывает.
...
Если метод вызывается в версии, обёрнутой через [`util.promisify()`](util.md#utilpromisifyoriginal), он возвращает промис, исполняемый объектом `Object` со свойствами `bytesRead` и `buffer`.
...
Аналогично [`fs.read()`](#fsreadfd-buffer-offset-length-position-callback), эта перегрузка принимает необязательный объект `options`. Если объект `options` не указан, используются значения по умолчанию, перечисленные выше.

### `fs.read(fd, buffer[, options], callback)`

-   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type)
-   `buffer` [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) Буфер, в который будут записаны прочитанные данные.
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `offset` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0`
    -   `length` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `buffer.byteLength - offset`
...
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
    -   `files` [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer[]>`](buffer.md#buffer) | [`<fs.Dirent[]>`](fs.md#class-fsdirent)
...
Если `options.withFileTypes` установлено в `true`, массив `files` будет содержать объекты [fs.Dirent](fs.md#class-fsdirent).

### `fs.readFile(path[, options], callback)`

-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) имя файла или файловый дескриптор
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | null **По умолчанию:** `null`
    -   `flag` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) см. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'r'`.
    -   `signal` [`<AbortSignal>`](globals.md#abortsignal) позволяет прервать выполняющийся `readFile`
...
Необязательный аргумент `options` может быть строкой с именем кодировки или объектом со свойством `encoding`, задающим кодировку символов для пути, передаваемого в callback. Если `encoding` равен `'buffer'`, путь в callback передаётся как объект [Buffer](buffer.md#buffer).
...
Асинхронная версия `stat(2)`. Callback получает два аргумента `(err, stats)`, где `stats` — объект [fs.Stats](fs.md#class-fsstats).

В случае ошибки `err.code` будет одним из [распространённых системных ошибок](errors.md#common-system-errors).

[`fs.stat()`](#fsstatpath-options-callback) разыменовывает символические ссылки. Чтобы получить сведения о самих ссылках, используйте [`fs.lstat()`](#fslstatpath-options-callback).
...
Чтобы проверить существование файла без последующих операций с ним, рекомендуется [`fs.access()`](#fsaccesspath-mode-callback).
...
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
    -   `stats` [`<fs.StatFs>`](fs.md#class-fsstatfs)

Асинхронная версия `statfs(2)`. Возвращает информацию о смонтированной файловой системе, содержащей `path`. Callback получает два аргумента `(err, stats)`, где `stats` — объект [fs.StatFs](fs.md#class-fsstatfs).
...
`fs.unlink()` не удаляет каталог — ни пустой, ни непустой. Чтобы удалить каталог, используйте [`fs.rmdir()`](#fsrmdirpath-options-callback).
...
Использование [`fs.watch()`](#fswatchfilename-options-listener) эффективнее, чем `fs.watchFile()` и `fs.unwatchFile()`. По возможности вместо `fs.watchFile()` и `fs.unwatchFile()` следует использовать `fs.watch()`.
...
-   Возвращает: [`<fs.FSWatcher>`](fs.md#class-fsfswatcher)
...
Callback-слушатель привязан к событию `'change'`, которое генерирует [fs.FSWatcher](fs.md#class-fsfswatcher), но это не то же самое, что значение `'change'` у `eventType`.

Если передан `signal`, вызов `abort()` у соответствующего `AbortController` закроет возвращённый [fs.FSWatcher](fs.md#class-fsfswatcher).
...
-   `listener` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `current` [`<fs.Stats>`](fs.md#class-fsstats)
    -   `previous` [`<fs.Stats>`](fs.md#class-fsstats)
-   Возвращает: [`<fs.StatWatcher>`](fs.md#class-fsstatwatcher)
...
-   `file` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) имя файла или файловый дескриптор
-   `data` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | null **По умолчанию:** `'utf8'`
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `flag` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) См. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'w'`.
    -   `flush` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если все данные успешно записаны в файл и `flush` равен `true`, для сброса буфера на диск вызывается `fs.fsync()`. **По умолчанию:** `false`.
    -   `signal` [`<AbortSignal>`](globals.md#abortsignal) позволяет прервать выполняющуюся операцию `writeFile`
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error) | [`<AggregateError>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/AggregateError)
...
Синхронно дописывает данные в файл; при отсутствии файла создаёт его. `data` может быть строкой или [Buffer](buffer.md#buffer).
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `dereference` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Разыменовывать символические ссылки. **По умолчанию:** `false`.
    -   `errorOnExist` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `force` равно `false` и место назначения существует, выбрасывать ошибку. **По умолчанию:** `false`.
    -   `filter` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Функция для фильтрации копируемых файлов и каталогов. Верните `true`, чтобы скопировать элемент, и `false`, чтобы его проигнорировать. Если игнорируется каталог, всё его содержимое также будет пропущено. **По умолчанию:** `undefined`.
        -   `src` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Исходный путь для копирования.
        -   `dest` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Путь назначения для копирования.
        -   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Любое значение (не `Promise`), приводимое к `boolean`.
    -   `force` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Перезаписывать существующий файл или каталог. Операция копирования будет игнорировать ошибки, если установить `false`, а место назначения уже существует. Используйте опцию `errorOnExist`, чтобы изменить это поведение. **По умолчанию:** `true`.
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Модификаторы операции копирования. **По умолчанию:** `0`. См. флаг `mode` у [`fs.copyFileSync()`](#fscopyfilesyncsrc-dest-mode).
    -   `preserveTimestamps` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, временные метки из `src` будут сохранены. **По умолчанию:** `false`.
    -   `recursive` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Копировать каталоги рекурсивно. **По умолчанию:** `false`
    -   `verbatimSymlinks` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, разрешение путей для символических ссылок будет пропущено. **По умолчанию:** `false`
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.exists()`](#fsexistspath-callback).
...
-   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type)
-   `uid` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) идентификатор пользователя (UID) нового владельца файла.
-   `gid` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) идентификатор группы (GID) новой группы файла.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) должны ли числовые значения в возвращаемом объекте [fs.Stats](fs.md#class-fsstats) иметь тип `bigint`. **По умолчанию:** `false`.
-   Возвращает: [`<fs.Stats>`](fs.md#class-fsstats)

Получает [fs.Stats](fs.md#class-fsstats) для файлового дескриптора.
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.ftruncate()`](#fsftruncatefd-len-callback).
...
Синхронный вариант [`fs.futimes()`](#fsfutimesfd-atime-mtime-callback). Возвращает `undefined`.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `exclude` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) | [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) Функция для исключения файлов и каталогов из выборки либо список glob-шаблонов для исключения. Если передана функция, верните `true`, чтобы исключить элемент, и `false`, чтобы его включить. **По умолчанию:** `undefined`. Если передан массив строк, каждая строка должна быть glob-шаблоном, задающим исключаемые пути. Примечание: шаблоны с отрицанием (например, `'!foo.js'`) не поддерживаются.
    -   `withFileTypes` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, glob возвращает пути в виде объектов `Dirent`; если `false` — в виде строк. **По умолчанию:** `false`.
-   Возвращает: [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) пути к файлам, соответствующим шаблону.
...
Изменяет временные метки файловой системы для символической ссылки, на которую указывает `path`. Возвращает `undefined` либо выбрасывает исключение при неверных параметрах или сбое операции. Это синхронный вариант [`fs.lutimes()`](#fslutimespath-atime-mtime-callback).
...
Получает [fs.Stats](fs.md#class-fsstats) для символической ссылки, на которую указывает `path`.
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.mkdtemp()`](#fsmkdtempprefix-options-callback).
...
-   `options` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) **По умолчанию:** `'utf8'`
-   Возвращает: [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) disposable-объект:
    -   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) путь созданного каталога
    -   `remove` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) функция, удаляющая созданный каталог
    -   `[Symbol.dispose]` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) то же самое, что `remove`
...
Подробнее см. документацию [`fs.mkdtemp()`](#fsmkdtempprefix-options-callback).
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api)
-   `flags` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `'r'`. См. [поддержку файловых `flags`](#file-system-flags).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.open()`](#fsopenpath-flags-mode-callback).
...
-   `options` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `withFileTypes` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `false`
    -   `recursive` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) если `true`, читает содержимое каталога рекурсивно. В рекурсивном режиме будут перечислены все файлы, вложенные файлы и каталоги. **По умолчанию:** `false`.
-   Возвращает: [`<string[]>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer[]>`](buffer.md#buffer) | [`<fs.Dirent[]>`](fs.md#class-fsdirent)

Читает содержимое каталога.

Подробнее см. POSIX readdir(3).

Необязательный аргумент `options` может быть строкой с именем кодировки или объектом со свойством `encoding`, задающим кодировку имён возвращаемых файлов. Если `encoding` равен `'buffer'`, имена возвращаются как объекты [Buffer](buffer.md#buffer).

Если `options.withFileTypes` равен `true`, в результате будут объекты [fs.Dirent](fs.md#class-fsdirent).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.readFile()`](#fsreadfilepath-options-callback).

Если указана опция `encoding`, эта функция возвращает строку. В противном случае она возвращает буфер.

Аналогично [`fs.readFile()`](#fsreadfilepath-options-callback): если `path` — каталог, поведение `fs.readFileSync()` зависит от платформы.
...
Необязательный аргумент `options` может быть строкой с именем кодировки или объектом со свойством `encoding`, задающим кодировку символов для возвращаемого пути ссылки. Если `encoding` равен `'buffer'`, путь возвращается как объект [Buffer](buffer.md#buffer).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.read()`](#fsreadfd-buffer-offset-length-position-callback).
...
-   `buffers` [`<ArrayBufferView[]>`](https://developer.mozilla.org/docs/Web/API/ArrayBufferView)
-   `position` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | null **По умолчанию:** `null`
-   Возвращает: [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Количество прочитанных байт.

Подробнее см. документацию асинхронного варианта этого API: [`fs.readv()`](#fsreadvfd-buffers-position-callback).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.realpath()`](#fsrealpathpath-options-callback).
...
Чтобы получить поведение, близкое к команде Unix `rm -rf`, используйте [`fs.rmSync()`](#fsrmsyncpath-options) с опциями `{ recursive: true, force: true }`.
...
Получает [fs.Stats](fs.md#class-fsstats) для указанного пути.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    -   `bigint` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) должны ли числовые значения в возвращаемом объекте [fs.StatFs](fs.md#class-fsstatfs) иметь тип `bigint`. **По умолчанию:** `false`.
-   Возвращает: [`<fs.StatFs>`](fs.md#class-fsstatfs)
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.symlink()`](#fssymlinktarget-path-type-callback).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.utimes()`](#fsutimespath-atime-mtime-callback).
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    ...
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `flag` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) См. [поддержку файловых `flags`](#file-system-flags). **По умолчанию:** `'w'`.
    -   `flush` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если все данные успешно записаны в файл и `flush` равен `true`, для сброса буфера на диск вызывается `fs.fsyncSync()`.
-   Возвращает: `undefined`.

Опция `mode` влияет только на вновь создаваемый файл. Подробнее см. [`fs.open()`](#fsopenpath-flags-mode-callback).

Подробнее см. документацию асинхронного варианта этого API: [`fs.writeFile()`](#fswritefilefile-data-options-callback).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.write(fd, buffer...)`](#fswritefd-buffer-offset-length-position-callback).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.write(fd, string...)`](#fswritefd-string-position-encoding-callback).
...
Подробнее см. документацию асинхронного варианта этого API: [`fs.writev()`](#fswritevfd-buffers-position-callback).
...
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise), который выполняется с [fs.Dirent](fs.md#class-fsdirent) или `null`

Асинхронно читает следующую запись каталога через `readdir(3)` как [fs.Dirent](fs.md#class-fsdirent).

Возвращает промис, который будет выполнен с [fs.Dirent](fs.md#class-fsdirent) или `null`, если больше нет записей каталога для чтения.
...
-   `callback` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)
    -   `err` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)
    -   `dirent` [`<fs.Dirent>`](fs.md#class-fsdirent) | null

Асинхронно читает следующую запись каталога через `readdir(3)` как [fs.Dirent](fs.md#class-fsdirent).

После завершения чтения `callback` будет вызван с [fs.Dirent](fs.md#class-fsdirent) или `null`, если больше нет записей каталога для чтения.

Записи каталога, возвращаемые этой функцией, не имеют определённого порядка и зависят от базовых механизмов каталогов операционной системы. Записи, добавленные или удалённые во время итерации по каталогу, могут не попасть в результаты итерации.

#### `dir.readSync()`

-   Возвращает: [`<fs.Dirent>`](fs.md#class-fsdirent) | null

Синхронно читает следующую запись каталога как [fs.Dirent](fs.md#class-fsdirent). Подробнее см. документацию POSIX `readdir(3)`.
...
-   Возвращает: [`<AsyncIterator>`](https://tc39.es/ecma262/#sec-asynciterator-interface) асинхронный итератор из [fs.Dirent](fs.md#class-fsdirent)

Асинхронно перебирает каталог, пока не будут прочитаны все записи. Подробнее см. документацию POSIX `readdir(3)`.

Записи, возвращаемые асинхронным итератором, всегда являются [fs.Dirent](fs.md#class-fsdirent). Случай `null` из `dir.read()` обрабатывается внутри.
...
Кроме того, когда [`fs.readdir()`](#fsreaddirpath-options-callback) или [`fs.readdirSync()`](#fsreaddirsyncpath-options) вызываются с опцией `withFileTypes`, установленной в `true`, результирующий массив заполняется объектами [fs.Dirent](fs.md#class-fsdirent), а не строками или [Buffer](buffer.md#buffer).

#### `dirent.isBlockDevice()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает блочное устройство.

#### `dirent.isCharacterDevice()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает символьное устройство.

#### `dirent.isDirectory()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает каталог файловой системы.

#### `dirent.isFIFO()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает канал FIFO (first-in-first-out).

#### `dirent.isFile()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает обычный файл.

#### `dirent.isSocket()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает сокет.

#### `dirent.isSymbolicLink()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Dirent](fs.md#class-fsdirent) описывает символическую ссылку.

#### `dirent.name`

-   Тип: [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer)

Имя файла, на который указывает этот объект [fs.Dirent](fs.md#class-fsdirent). Тип этого значения определяется `options.encoding`, переданным в [`fs.readdir()`](#fsreaddirpath-options-callback) или [`fs.readdirSync()`](#fsreaddirsyncpath-options).

#### `dirent.parentPath`

-   Тип: [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)

Путь к родительскому каталогу файла, на который указывает этот объект [fs.Dirent](fs.md#class-fsdirent).

### Класс: `fs.FSWatcher` {#class-fsfswatcher}

-   Наследует [`<EventEmitter>`](events.md#class-eventemitter)

Успешный вызов метода [`fs.watch()`](#fswatchfilename-options-listener) возвращает новый объект [fs.FSWatcher](fs.md#class-fsfswatcher).

Все объекты [fs.FSWatcher](fs.md#class-fsfswatcher) генерируют событие `'change'` всякий раз, когда изменяется конкретный отслеживаемый файл.
...
Генерируется, когда наблюдатель прекращает следить за изменениями. Закрытый объект [fs.FSWatcher](fs.md#class-fsfswatcher) больше нельзя использовать в обработчике события.

#### Событие: `'error'`

-   `error` [`<Error>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error)

Генерируется при возникновении ошибки во время наблюдения за файлом. Объект [fs.FSWatcher](fs.md#class-fsfswatcher), для которого произошла ошибка, больше нельзя использовать в обработчике события.

#### `watcher.close()`

Прекращает наблюдение за изменениями для данного [fs.FSWatcher](fs.md#class-fsfswatcher). После остановки объект [fs.FSWatcher](fs.md#class-fsfswatcher) больше нельзя использовать.

#### `watcher.ref()`

-   Возвращает: [`<fs.FSWatcher>`](fs.md#class-fsfswatcher)

При вызове запрашивает, чтобы цикл событий Node.js _не_ завершался, пока активен [fs.FSWatcher](fs.md#class-fsfswatcher). Многократный вызов `watcher.ref()` ни на что не влияет.

По умолчанию все объекты [fs.FSWatcher](fs.md#class-fsfswatcher) уже имеют состояние "ref", поэтому обычно вызывать `watcher.ref()` не требуется, если ранее не вызывался `watcher.unref()`.

#### `watcher.unref()`

-   Возвращает: [`<fs.FSWatcher>`](fs.md#class-fsfswatcher)

При вызове активный объект [fs.FSWatcher](fs.md#class-fsfswatcher) перестаёт требовать, чтобы цикл событий Node.js оставался активным. Если нет другой активности, поддерживающей цикл событий, процесс может завершиться до вызова callback объекта [fs.FSWatcher](fs.md#class-fsfswatcher). Многократный вызов `watcher.unref()` ни на что не влияет.

### Класс: `fs.StatWatcher` {#class-fsstatwatcher}

-   Наследует [`<EventEmitter>`](events.md#class-eventemitter)

Успешный вызов метода `fs.watchFile()` возвращает новый объект [fs.StatWatcher](fs.md#class-fsstatwatcher).

#### `watcher.ref()`

-   Возвращает: [`<fs.StatWatcher>`](fs.md#class-fsstatwatcher)

При вызове запрашивает, чтобы цикл событий Node.js _не_ завершался, пока активен [fs.StatWatcher](fs.md#class-fsstatwatcher). Многократный вызов `watcher.ref()` ни на что не влияет.

По умолчанию все объекты [fs.StatWatcher](fs.md#class-fsstatwatcher) уже имеют состояние "ref", поэтому обычно вызывать `watcher.ref()` не требуется, если ранее не вызывался `watcher.unref()`.

#### `watcher.unref()`

-   Возвращает: [`<fs.StatWatcher>`](fs.md#class-fsstatwatcher)

При вызове активный объект [fs.StatWatcher](fs.md#class-fsstatwatcher) перестаёт требовать, чтобы цикл событий Node.js оставался активным. Если нет другой активности, поддерживающей цикл событий, процесс может завершиться до вызова callback объекта [fs.StatWatcher](fs.md#class-fsstatwatcher). Многократный вызов `watcher.unref()` ни на что не влияет.
...
Объект [fs.Stats](fs.md#class-fsstats) содержит информацию о файле.
...
Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает блочное устройство.

#### `stats.isCharacterDevice()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает символьное устройство.

#### `stats.isDirectory()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает каталог файловой системы.

Если объект [fs.Stats](fs.md#class-fsstats) получен вызовом [`fs.lstat()`](#fslstatpath-options-callback) для символической ссылки, которая указывает на каталог, этот метод вернёт `false`. Это связано с тем, что [`fs.lstat()`](#fslstatpath-options-callback) возвращает информацию о самой символической ссылке, а не о пути, к которому она разрешается.

#### `stats.isFIFO()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает FIFO-канал (first-in-first-out).

#### `stats.isFile()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает обычный файл.

#### `stats.isSocket()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает сокет.

#### `stats.isSymbolicLink()`

-   Возвращает: [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type)

Возвращает `true`, если объект [fs.Stats](fs.md#class-fsstats) описывает символическую ссылку.
...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `sync`: [boolean](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) выполнять запись синхронно.
...
-   Наследует [`<stream.Writable>`](stream.md#class-streamwritable)

Экземпляры [fs.WriteStream](fs.md#class-fswritestream) нельзя создавать напрямую. Они создаются и возвращаются функцией [`fs.createWriteStream()`](#fscreatewritestreampath-options).

#### Событие: `'close'`

Генерируется, когда базовый файловый дескриптор [fs.WriteStream](fs.md#class-fswritestream) закрыт.

#### Событие: `'open'`

-   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) целочисленный файловый дескриптор, используемый [fs.WriteStream](fs.md#class-fswritestream)

Генерируется, когда файл [fs.WriteStream](fs.md#class-fswritestream) открыт.

#### Событие: `'ready'`

Генерируется, когда [fs.WriteStream](fs.md#class-fswritestream) готов к использованию.
...
Следующие константы предназначены для использования со свойством `mode` объекта [fs.Stats](fs.md#class-fsstats) для определения типа файла.
...
Следующие константы предназначены для использования со свойством `mode` объекта [fs.Stats](fs.md#class-fsstats) для определения прав доступа к файлу.
...
API на основе промисов используют объект [FileHandle](#class-filehandle) вместо числового файлового дескриптора. Система лучше управляет такими объектами, чтобы предотвратить утечки ресурсов. Тем не менее, по завершении операций их всё равно необходимо закрывать:
