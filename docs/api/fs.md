...
Callback-слушатель привязан к событию `'change'`, которое генерирует [fs.FSWatcher](fs.md#fsfswatcher), но это не то же самое, что значение `'change'` у `eventType`.

Если передан `signal`, вызов `abort()` у соответствующего `AbortController` закроет возвращённый [fs.FSWatcher](fs.md#fsfswatcher).
...
Если `options.withFileTypes` равен `true`, в результате будут объекты [fs.Dirent](fs.md#fsdirent).
...
-   Наследует [`<stream.Writable>`](stream.md#streamwritable)
