...
Поведение при дублирующихся заголовках см. в [`message.headers`][`message.headers`].
...
(см. [`socket.unref()`][`socket.unref()`]).

Рекомендуется вызывать [`destroy()`][`destroy()`] у `Agent`, когда он больше не нужен: лишние
...
Чтобы изменить эти параметры, нужно создать собственный экземпляр [`http.Agent`][`http.Agent`].
...
По умолчанию поведение совпадает с [`net.createConnection()`][`net.createConnection()`]: сокет создаётся
...
менять через [`setHeader(name, value)`][`setHeader(name, value)`], [`getHeader(name)`][`getHeader(name)`],
[`removeHeader(name)`][`removeHeader(name)`]. Фактический заголовок уйдёт с первым фрагментом данных
или при вызове [`request.end()`][`request.end()`].

Чтобы получить ответ, добавьте обработчик [`'response'`][`'response'`] к объекту запроса.
[`'response'`][`'response'`] возникает, когда получены заголовки ответа; с одним аргументом —
экземпляром [`http.IncomingMessage`][`http.IncomingMessage`].

Во время [`'response'`][`'response'`] можно подписаться на объект ответа, в частности на
`'data'`.

Если обработчик [`'response'`][`'response'`] не добавлен, ответ полностью отбрасывается. Если
...
Если [`response.strictContentLength`][`response.strictContentLength`] равен `true`, несоответствие значения
`Content-Length` приведёт к `Error` с кодом [`'ERR_HTTP_CONTENT_LENGTH_MISMATCH'`][`'ERR_HTTP_CONTENT_LENGTH_MISMATCH'`].

`Content-Length` задаётся в байтах, не в символах. Для длины тела в байтах
используйте [`Buffer.byteLength()`][`Buffer.byteLength()`].
...
слушайте событие [`'upgrade'`][`'upgrade'`].
...
простое; запрос нужно завершить вручную (например [`request.destroy()`][`request.destroy()`]).

См. также [`request.setTimeout()`][`request.setTimeout()`].
...
> Stability: 0 - Deprecated: Use [`request.destroy()`][`request.destroy()`] instead.
...
См. [`writable.cork()`][`writable.cork()`].
...
[`request.write(data, encoding)`][`request.write(data, encoding)`] с последующим `request.end(callback)`.
...
[`request.getHeader()`][`request.getHeader()`] может вернуть не строку; при передаче по сети значения
...
См. [`writable.uncork()`][`writable.uncork()`].
...
нужен вызов [`request.end()`][`request.end()`].
...
Обработка: вызвать [`response.writeContinue()`][`response.writeContinue()`], если клиенту следует
продолжить отправку тела, или сформировать подходящий HTTP-ответ (например
400 Bad Request), если тело отправлять не нужно.

Если событие обработано, событие [`'request'`][`'request'`] не генерируется.
...
или `431 Request Header Fields Too Large` при ошибке [`HPE_HEADER_OVERFLOW`][`HPE_HEADER_OVERFLOW`].
Если сокет не доступен для записи или уже отправлены заголовки привязанного
[`http.ServerResponse`][`http.ServerResponse`], сокет сразу уничтожается.

`socket` — это [`net.Socket`][`net.Socket`], с которого произошла ошибка.
...
duplex. Сырое соединение — в [`request.socket`][`request.socket`] (экземпляр [net.Socket](net.md#class-netsocket), если
...
См. [`net.Server.close()`][`net.Server.close()`].
...
* Тип: [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** меньшее из [`server.requestTimeout`][`server.requestTimeout`] и `60000`.
...
Запускает прослушивание HTTP-соединений. Аналог [`server.listen()`][`server.listen()`] у [`net.Server`][`net.Server`].
...
К этому значению добавляется [`server.keepAliveTimeoutBuffer`][`server.keepAliveTimeoutBuffer`]:
`socketTimeout = keepAliveTimeout + keepAliveTimeoutBuffer`.
Если до срабатывания keep-alive таймаута приходят новые данные, сбрасывается
обычный таймаут простоя — [`server.timeout`][`server.timeout`].
...
Вызывает [`server.close()`][`server.close()`] и возвращает промис, выполняющийся после закрытия сервера.
...
Недопустимые символы в имени или значении заголовка приводят к [`TypeError`][`TypeError`].
...
Если задан `data`, это аналогично [`response.write(data, encoding)`][`response.write(data, encoding)`] с
...
Немедленно отправляет заголовки ответа. См. также [`request.flushHeaders()`][`request.flushHeaders()`].
...
Нестроковые значения хранятся как есть, [`response.getHeader()`][`response.getHeader()`] может вернуть
...
Недопустимые символы в имени или значении приводят к [`TypeError`][`TypeError`].

Заголовки из [`response.setHeader()`][`response.setHeader()`] объединяются с заголовками
[`response.writeHead()`][`response.writeHead()`]; приоритет у аргументов [`response.writeHead()`][`response.writeHead()`].
...
Если вызван [`response.writeHead()`][`response.writeHead()`] без предварительных [`response.setHeader()`][`response.setHeader()`],
значения заголовков пишутся в канал без внутреннего кэша, и [`response.getHeader()`][`response.getHeader()`]
может не показать ожидаемое. Если нужно поэтапно задавать и потом читать/менять
заголовки, используйте [`response.setHeader()`][`response.setHeader()`], а не только [`response.writeHead()`][`response.writeHead()`].
...
Первый вызов [`response.write()`][`response.write()`] отправляет буферизованные заголовки и первый
...
запроса. См. событие [`'checkContinue'`][`'checkContinue'`] на `Server`.
...
Вызывать не более одного раза на сообщение и до [`response.end()`][`response.end()`].

Если до этого вызывались [`response.write()`][`response.write()`] или [`response.end()`][`response.end()`], будут
вычислены неявные/изменяемые заголовки и вызвана эта функция.

Заголовки из [`response.setHeader()`][`response.setHeader()`] объединяются с аргументами
[`response.writeHead()`][`response.writeHead()`]; приоритет у [`response.writeHead()`][`response.writeHead()`].

Если вызван [`response.writeHead()`][`response.writeHead()`] без предварительных [`response.setHeader()`][`response.setHeader()`],
значения пишутся в канал без внутреннего кэша, [`response.getHeader()`][`response.getHeader()`] может не
...
[`Buffer.byteLength()`][`Buffer.byteLength()`]. Node.js проверяет согласованность `Content-Length` с
переданным телом.

Недопустимые символы в имени или значении заголовка — [`TypeError`][`TypeError`].
...
> Stability: 0 - Deprecated. Use [`message.socket`][`message.socket`].

Alias for [`message.socket`][`message.socket`].
...
* Повторы `age`, `authorization`, `content-length`, `content-type`,
  ...
  этих заголовков, используйте `joinDuplicateHeaders` в [`http.request()`][`http.request()`]
  и [`http.createServer()`][`http.createServer()`] (см. RFC 9110 раздел 5.3).
...
При HTTPS можно вызвать [`request.socket.getPeerCertificate()`][`request.socket.getPeerCertificate()`] для данных
сертификата клиента.
...
Alias of [`outgoingMessage.socket`][`outgoingMessage.socket`].
...
Заголовки из [`outgoingMessage.setHeaders()`][`outgoingMessage.setHeaders()`] объединяются с
[`response.writeHead()`][`response.writeHead()`]; приоритет у [`response.writeHead()`][`response.writeHead()`].
...
`requestListener` автоматически добавляется как обработчик [`'request'`][`'request'`].
...
прочитать, как описано в разделе [`http.ClientRequest`][`http.ClientRequest`].

`callback` вызывается с одним аргументом — экземпляром [`http.IncomingMessage`][`http.IncomingMessage`].
...
`url` — строка или объект [`URL`][`URL`]; строка разбирается через [`new URL()`][`new URL()`],
объект [`URL`][`URL`] превращается в обычный `options`.

Если заданы и `url`, и `options`, объекты сливаются; поля из `options` важнее.

Необязательный `callback` добавляется как одноразовый слушатель [`'response'`][`'response'`].

`http.request()` возвращает [`http.ClientRequest`][`http.ClientRequest`] — поток для записи.
...
Недопустимое `name` даёт [`TypeError`][`TypeError`] с `code: 'ERR_INVALID_HTTP_TOKEN'`.
...
Недопустимое значение даёт [`TypeError`][`TypeError`].
...
* `proxyEnv` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) Конфигурация прокси; те же поля, что у опции `proxyEnv` у
  [`Agent`][`Agent`]. **По умолчанию:** `process.env`.
...
Динамически включить прокси глобально: [`http.setGlobalProxyFromEnv()`][`http.setGlobalProxyFromEnv()`].
