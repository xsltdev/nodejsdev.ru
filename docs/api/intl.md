...
-   Функции, чувствительные к локали или осведомлённые об Unicode, из [спецификации языка ECMAScript](https://tc39.github.io/ecma262/):
    -   [`String.prototype.normalize()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/normalize)
    -   [`String.prototype.toLowerCase()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/toLowerCase)
    -   [`String.prototype.toUpperCase()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/toUpperCase)
-   Вся функциональность, описанная в [спецификации ECMAScript Internationalization API](https://tc39.github.io/ecma402/) (она же ECMA-402):
    -   объект [`Intl`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl)
    -   методы, чувствительные к локали, например [`String.prototype.localeCompare()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/localeCompare) и [`Date.prototype.toLocaleString()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toLocaleString)
-   Поддержка [интернационализированных доменных имён](https://en.wikipedia.org/wiki/Internationalized_domain_name) (IDN) в [парсере URL WHATWG](url.md#the-whatwg-url-api)
-   [`require('node:buffer').transcode()`](buffer.md#buffertranscodesource-fromenc-toenc)
-   Более точное редактирование строк в [REPL](repl.md#repl)
-   [`require('node:util').TextDecoder`](util.md#class-utiltextdecoder)
-   [`RegExp` Unicode Property Escapes](https://github.com/tc39/proposal-regexp-unicode-property-escapes)

Node.js и лежащий в основе движок V8 используют [International Components for Unicode (ICU)](http://site.icu-project.org/) для реализации этих возможностей в нативном коде на C/C++. Полный набор данных ICU поставляется с Node.js по умолчанию. Однако из‑за размера файла данных ICU при сборке или запуске Node.js доступны несколько вариантов настройки набора данных ICU.

## Опции сборки Node.js

Чтобы управлять использованием ICU в Node.js, при компиляции доступны четыре опции `configure`. Дополнительные сведения о сборке Node.js описаны в [BUILDING.md](https://github.com/nodejs/node/blob/HEAD/BUILDING.md).
...
| [`Number.prototype.toLocaleString()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/toLocaleString) | частичная (без учёта локали) | частичная/полная (зависит от ОС) | частичная (только английский) | полная |
...
| [Legacy URL Parser](url.md#legacy-url-api) | частичная (без поддержки IDN) | полная | полная | полная |
| [WHATWG URL Parser](url.md#the-whatwg-url-api) | частичная (без поддержки IDN) | полная | полная | полная |
| [`require('node:buffer').transcode()`](buffer.md#buffertranscodesource-fromenc-toenc) | нет (функция отсутствует) | полная | полная | полная |
| [REPL](repl.md#repl) | частичная (неточное редактирование строк) | полная | полная | полная |
| [`require('node:util').TextDecoder`](util.md#class-utiltextdecoder) | частичная (базовые кодировки) | частичная/полная (зависит от ОС) | частичная (только Unicode) | полная |
| [`RegExp` Unicode Property Escapes](https://github.com/tc39/proposal-regexp-unicode-property-escapes) | нет (ошибка недопустимого `RegExp`) | полная | полная | полная |
...
Функции, которым нужна только библиотека ICU, например [`String.prototype.normalize()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/normalize) и [парсер URL WHATWG](url.md#the-whatwg-url-api), при `system-icu` поддерживаются полностью. Возможности, которым дополнительно нужны локальные данные ICU, например [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat), _могут_ быть полностью или частично поддержаны в зависимости от полноты данных ICU в системе.
...
Функции, которым нужна только библиотека ICU, например [`String.prototype.normalize()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/normalize) и [парсер URL WHATWG](url.md#the-whatwg-url-api), при `small-icu` поддерживаются полностью. Возможности, которым дополнительно нужны локальные данные ICU, например [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat), как правило работают только с английской локалью:
...
-   Переменная окружения [`NODE_ICU_DATA`](cli.md#node_icu_datafile):

    ```bash
    env NODE_ICU_DATA=/runtime/directory/with/dat/file node
    ```

-   Параметр CLI [`--icu-data-dir`](cli.md#--icu-data-dirfile):
...
}${os.endianness()[0].toLowerCase()}.dat`;
```

См. статью [«ICU Data»](http://userguide.icu-project.org/icudata) в ICU User Guide про другие поддерживаемые форматы и подробности про данные ICU в целом.

Модуль npm [full-icu](https://www.npmjs.com/package/full-icu) сильно упрощает установку данных ICU: он определяет версию ICU у запущенного `node` и скачивает подходящий файл. После `npm i full-icu` файл будет в `./node_modules/full-icu`. Этот путь можно передать в `NODE_ICU_DATA` или `--icu-data-dir`, как показано выше, чтобы включить полную поддержку `Intl`.
...
Чтобы проверить поддержку неанглийской локали (т.е. `full-icu` или `system-icu`), хорошо подходит [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat):
...
-   [btest402](https://github.com/srl295/btest402): обычно используют, чтобы проверить, что Node.js с поддержкой `Intl` собран корректно.
-   [Test262](https://github.com/tc39/test262/tree/HEAD/test/intl402): официальный набор тестов соответствия ECMAScript включает раздел, посвящённый ECMA-402.
