...
[`node-addon-api`][`node-addon-api`] — официальная C++-обёртка, позволяющая эффективнее писать C++-код, вызывающий Node-API. Это header-only библиотека с подключаемым C++ API. Собранные с `node-addon-api` бинарники зависят от символов C-функций Node-API, экспортируемых Node.js. Ниже пример кода `node-addon-api`:
