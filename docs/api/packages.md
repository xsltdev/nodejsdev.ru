...
По умолчанию Corepack не будет применять какой-либо конкретный менеджер пакетов и будет использовать общие версии "Last Known Good", связанные с каждым выпуском Node.js, но вы можете улучшить этот опыт, установив поле [``пакетМенеджер``](#packagemanager) в `package.json` вашего проекта.
...
Поле [`"main"`](#main) поддерживается во всех версиях Node.js, но его возможности ограничены: оно определяет только главную точку входа пакета.
...
Существующие пакеты, вводящие поле [``экспорт``](#exports), не позволят потребителям пакета использовать любые точки входа, которые не определены, включая [``package.json``](#nodejs-packagejson-field-definitions) (например, `require('your-package/package.json')`). **Это, вероятно, будет ломающим изменением*.


Чтобы сделать введение [``экспорта``](#exports) не ломающим, убедитесь, что каждая ранее поддерживаемая точка входа экспортируется. Лучше всего явно указать точки входа, чтобы публичный API пакета был четко определен. Например, проект, который ранее экспортировал `main`, `lib`, `feature` и `package.json`, может использовать следующий `package.exports`:
...
При написании нового пакета рекомендуется использовать поле [`` exports``](#exports):
...
Когда определено поле [`` exports``](#exports), все подпакеты пакета инкапсулируются и больше не доступны для импортеров. Например, `require('pkg/subpath.js')` вызывает ошибку [`ERR_PACKAGE_PATH_NOT_EXPORTED`](errors.md#err_package_path_not_exported).
...
Определяет пакет, где `require('pkg/feature.js')` и `import 'pkg/feature.js'` cou
...
что позволит разрешить условие ``разработка`` в импорте и экспорте пакетов, одновременно разрешая существующие условия ``node``, ``node-addons``, `` default``, `` import`` и `` require``.
...
Поскольку пользовательские условия пакетов требуют четких определений для обеспечения правильного использования, ниже приведен список общих известных условий пакетов и их строгих определений для помощи в координации экосистемы.
  ...
  - ``Типы`` - может использоваться системами типизации для разрешения файла типизации для данного экспорта. *Это условие всегда должно быть включено первым.*
  - ``дено`` - указывает на вариацию для платформы Deno.
  - `` browser`` - любая среда веб-браузера.
  - `` react-native`` - будет соответствовать фреймворку React Native (все платформы). *Чтобы нацелить React Native для Web, ``браузер`` должен быть указан перед этим условием.
  - ``разработка`` - может использоваться для определения точки входа в среду только для разработки, например, для обеспечения дополнительного отладочного контекста, такого как лучшие сообщения об ошибках при запуске в режиме разработки. *Всегда должно быть взаимоисключающим с `` production``.
...
Поле ``имя`` определяет имя вашего пакета. Для публикации в реестре *npm* требуется имя, удовлетворяющее [определенным требованиям] (https://docs.npmjs.com/files/package.json#name).
...
Ближайший родительский `package.json` определяется как первый `package.json`, найденный при поиске в текущей папке, родительской папке этой папки и так далее, пока не будет достигнута папка node\_modules или корень тома.
