...
Node.js будет считать следующее [ES-модулями](esm.md), если передать это в `node` как начальный ввод или сослаться через операторы `import` или выражения `import()`:
...
-   код, который синтаксически разбирается только как [ES-модули](esm.md), например операторы `import`/`export` или `import.meta`, без явного указания интерпретации. Явные маркеры — расширения `.mjs`/`.cjs`, поле `"type"` в `package.json` со значениями `"module"` или `"commonjs"` или флаг `--input-type`. Динамические выражения `import()` допустимы и в CommonJS, и в ES-модулях и сами по себе не заставляют файл считаться ES-модулем. См. [обнаружение синтаксиса](#syntax-detection).

Node.js будет считать следующее [CommonJS](modules.md), если передать это в `node` как начальный ввод или сослаться через `import`/`import()`:
...
-   Разрешение:
    -   разрешение через `require()` поддерживает [папки как модули](modules.md#folders-as-modules);
    -   при отсутствии точного совпадения `require()` подставляет расширения (`.js`, `.json`, затем `.node`) и снова пытается разрешить [папки как модули](modules.md#folders-as-modules);
...
-   Загрузка:
    ...
    -   `.ts`, `.mts`, `.cts` — как [TypeScript](typescript.md);
    -   прочие расширения или отсутствие расширения — как JavaScript-текст;
    -   `require()` может [загружать ES-модули из CommonJS](modules.md#loading-ecmascript-modules-using-require) только если [ES-модуль](esm.md) _и его зависимости_ синхронны (нет top-level `await`).
...
-   Загрузка:
    ...
    -   `.wasm` — [WebAssembly-модули](esm.md#wasm-modules);
    -   иные расширения дают ошибку [`ERR_UNKNOWN_FILE_EXTENSION`](errors.md#err_unknown_file_extension); дополнительные расширения — через [хуки настройки](module.md#customization-hooks);
    -   `import`/`import()` могут загружать JavaScript-[модули CommonJS](modules.md); для них используется [merve](https://github.com/anonrig/merve), чтобы по возможности вывести именованные экспорты статическим анализом.

Независимо от способа запроса разрешение и загрузку можно настроить через [хуки настройки](module.md#customization-hooks).

### `package.json` и расширения файлов

В пакете поле [`package.json`](#nodejs-packagejson-field-definitions) [`"type"`](#type) задаёт, как Node.js интерпретирует файлы `.js`. Без `"type"` файлы `.js` считаются [CommonJS](modules.md).

`"type": "module"` означает, что `.js` в этом пакете — [ES module](esm.md).
...
Файлы `.mjs` всегда загружаются как [ES modules](esm.md), независимо от родительского `package.json`.

Файлы `.cjs` всегда загружаются как [CommonJS](modules.md), независимо от родительского `package.json`.
...
-   в пакете с `"type": "module"` отдельный файл можно пометить как [CommonJS](modules.md), давав ему `.cjs` (и `.js`, и `.mjs` в таком пакете — ES-модули);

-   в пакете с `"type": "commonjs"` отдельный файл можно пометить как [ES module](esm.md), давав ему `.mjs` (и `.js`, и `.cjs` в таком пакете — CommonJS).

### Флаг `--input-type`

Строки для `--eval`/`-e` или из `STDIN` обрабатываются как [ES modules](esm.md), если задано `--input-type=module`.
...
[Условные экспорты](#conditional-exports) внутри [`"exports"`](#exports) задают разные точки входа по среде, включая `require` и `import`. Про совместимый CommonJS и ES в одном пакете см. [раздел про dual-пакеты](#dual-commonjses-module-packages).
...
Теперь, когда [карты импорта](https://github.com/WICG/import-maps) задают стандарт разрешения пакетов в браузерах и других средах JavaScript, стиль без расширения может раздувать определения карт импорта. Явные расширения файлов помогают избежать этого, позволяя карте импорта использовать [отображение папки пакетов](https://github.com/WICG/import-maps#packages-via-trailing-slashes) и по возможности сопоставлять несколько подпутей вместо отдельной записи карты на каждый экспортируемый подпуть. Это же согласуется с требованием указывать [полный путь спецификатора](esm.md#mandatory-file-extensions) в относительных и абсолютных спецификаторах импорта.
...
Использование условий `"import"` и `"require"` связано с рисками; подробнее — в [разделе о двойных пакетах CommonJS/ES-модулей](#dual-commonjses-module-packages).
...
Для прочих сред определения платформенных ключей ведёт [WinterCG](https://wintercg.org/) в спецификации предложения [Runtime Keys](https://runtime-keys.proposal.wintercg.org/).

Новые определения условий можно добавить в этот список через pull request в [документацию Node.js по этому разделу](https://github.com/nodejs/node/blob/HEAD/doc/api/packages.md#conditions-definitions). Требования к включению нового определения:
...
-   Пользователь Node.js ожидал бы увидеть условие в документации ядра. Хороший пример — `"types"`: оно не очень подходит к предложению [Runtime Keys](https://runtime-keys.proposal.wintercg.org/), но хорошо смотрится в документации Node.js.
...
Подробности — в [репозитории примеров пакетов](https://github.com/nodejs/package-examples).
...
Если у ближайшего родительского `package.json` нет поля `"type"` или указано `"type": "commonjs"`, файлы `.js` обрабатываются как [CommonJS](modules.md). Если дошли до корня тома и `package.json` не найден, файлы `.js` тоже считаются [CommonJS](modules.md).
...
Поле `"exports"` задаёт [точки входа](#package-entry-points) пакета при импорте по имени через поиск в `node_modules` или через [самоссылку](#self-referencing-a-package-using-its-name) по имени пакета. Поддерживается в Node.js 12+ как альтернатива [`"main"`](#main): можно описать [экспорт подпутей](#subpath-exports) и [условные экспорты](#conditional-exports), скрывая внутренние неэкспортируемые модули.
