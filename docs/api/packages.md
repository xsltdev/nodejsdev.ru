...
* код, который синтаксически разбирается только как [ES-модули][ES modules], например
  ...
  См. [Syntax detection][Syntax detection].
...
[Conditional exports][Conditional exports] внутри [`"exports"`][`"exports"`] задают разные точки входа по среде,
...
When using the [`"exports"`][`"exports"`] field, custom subpaths can be defined along
...
With [import maps][import maps] now providing a standard for package resolution in browsers
...
enabling the import map to utilize a [packages folder mapping][packages folder mapping] to map multiple
subpaths where possible instead of a separate map entry per package subpath
export. This also mirrors the requirement of using [the full specifier path][the full specifier path]
in relative and absolute import specifiers.

#### Path Rules and Validation for Export Targets

When defining paths as targets in the [`"exports"`][`"exports"`] field, Node.js enforces
...
All target paths in the [`"exports"`][`"exports"`] map (the values associated with export
...
If the `"."` export is the only export, the [`"exports"`][`"exports"`] field provides sugar
...
In addition to the [`"exports"`][`"exports"`] field, there is a package `"imports"` field
...
which are further explained in [the dual CommonJS/ES module packages section][the dual CommonJS/ES module packages section].

The `"node-addons"` condition can be used to provide an entry point which
uses native C++ addons. However, this condition can be disabled via the
[`--no-addons` flag][`--no-addons` flag]. When using `"node-addons"`, it's recommended to treat
...
enabled in Node.js via the [`--conditions` / `-C` flag][`--conditions` / `-C` flag].
...
by the [WinterCG][WinterCG] in the [Runtime Keys][Runtime Keys] proposal specification.

New conditions definitions may be added to this list by creating a pull request
to the [Node.js documentation for this section][Node.js documentation for this section]. The requirements for listing
...
* The condition should be such that a Node.js user would expect it to be in
  ...
  doesn't really belong in the [Runtime Keys][Runtime Keys] proposal but is a good fit
...
See [the package examples repository][the package examples repository] for details.
...
[self-reference][self-reference] a package using its name.
...
`"type": "commonjs"`, `.js` files are treated as [CommonJS][CommonJS]. If the volume
...
The `"exports"` field allows defining the [entry points][entry points] of a package when
imported by name loaded either via a `node_modules` lookup or a
[self-reference][self-reference] to its own name. It is supported in Node.js 12+ as an
...
This field defines [subpath imports][subpath imports] for the current package.
