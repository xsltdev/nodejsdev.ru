...
модуль [Punycode.js][]. Для кодирования URL на основе Punycode см.
[`url.domainToASCII`][] или, в более общем случае, [WHATWG URL API][].

Модуль `punycode` — это встроенная версия модуля [Punycode.js][]. Доступ к нему:

```js
const punycode = require('node:punycode');
```

[Punycode][] — схема кодирования символов по RFC 3492, в первую очередь предназначенная
...
Исправления и изменения нужно направлять в проект [Punycode.js][].
...
Метод `punycode.decode()` преобразует строку [Punycode][] из символов ASCII в
...
доменным именем в [Punycode][]. Преобразуются только не-ASCII части имени.
...
в [Punycode][] частями в Unicode. Преобразуются только части в Punycode.
...
Возвращает строку с номером текущей версии [Punycode.js][].
