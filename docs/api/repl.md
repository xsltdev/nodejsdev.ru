...
Модуль `node:repl` экспортирует класс [`repl.REPLServer`][`repl.ReplServer`]. Во время работы
экземпляры [`repl.REPLServer`][`repl.ReplServer`] принимают построчный ввод пользователя,
...
Экземпляры [`repl.REPLServer`][`repl.ReplServer`] поддерживают автодополнение, предпросмотр дополнений,
...
Остальные — в [TTY keybindings][TTY keybindings].

### Оценка по умолчанию

По умолчанию все экземпляры [`repl.REPLServer`][`repl.ReplServer`] используют функцию оценки,
которая вычисляет выражения JavaScript и даёт доступ к встроенным модулям Node.js.
Поведение можно переопределить, передав другую функцию оценки при создании [`repl.REPLServer`][`repl.ReplServer`].
...
REPL использует модуль [`domain`][`domain`], чтобы перехватывать все необработанные исключения сессии.

Такое использование [`domain`][`domain`] в REPL даёт такие эффекты:

* Событие [`'uncaughtException'`][`'uncaughtException'`] для необработанных исключений испускается только в
  автономном REPL. Подписка на это событие внутри REPL, встроенного в другую программу Node.js,
  даёт [`ERR_INVALID_REPL_INPUT`][`ERR_INVALID_REPL_INPUT`].
...
* Вызов [`process.setUncaughtExceptionCaptureCallback()`][`process.setUncaughtExceptionCaptureCallback()`] выбрасывает
...
Флаг [`--no-experimental-repl-await`][`--no-experimental-repl-await`] отключает top-level await в REPL.
...
При создании [`repl.REPLServer`][`repl.ReplServer`] можно передать свою функцию оценки —
...
По умолчанию [`repl.REPLServer`][`repl.ReplServer`] форматирует вывод через [`util.inspect()`][`util.inspect()`]
...
`inspect.replDefaults` — зеркало `defaultOptions` из [`util.inspect()`][`util.inspect()`].
...
Чтобы полностью задать вывод [`repl.REPLServer`][`repl.ReplServer`], передайте свою функцию в опции `writer`.
...
Пример «полноценного» (terminal) REPL на [`net.Server`][`net.Server`] и [`net.Socket`][`net.Socket`].
...
Пример запуска экземпляра REPL через [`curl()`][`curl()`].

Скрипт поднимает HTTP-сервер на порту `8000`, соединение можно установить через [`curl()`][`curl()`].
