...
Флаг [`--no-experimental-repl-await`](cli.md#--no-experimental-repl-await) отключает top-level await в REPL.
...
Пример «полноценного» (terminal) REPL на [`net.Server`](net.md#class-netserver) и [`net.Socket`](net.md#class-netsocket).
