...
Добавлена поддержка [`Worker`][`Worker`]. Подробнее — в разделе «Взаимодействие с worker» ниже на этой странице.
...
Подробное описание API см. в разделе [`документация process`][`документация process`].
...
Потоки [`Worker`][`Worker`] могут формировать отчёты так же, как основной поток.
