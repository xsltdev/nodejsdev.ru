...
Ресурсы задаются словарём «ключ — путь» в поле `assets`. На этапе сборки Node.js читает файлы по указанным путям и включает их в подготовительный блоб. В собранном исполняемом файле ресурсы доступны через API [`sea.getAsset()`][`sea.getAsset()`] и [`sea.getAssetAsBlob()`][`sea.getAssetAsBlob()`].
...
Подробнее см. [`sea.getAsset()`][`sea.getAsset()`], [`sea.getAssetAsBlob()`][`sea.getAssetAsBlob()`], [`sea.getRawAsset()`][`sea.getRawAsset()`] и [`sea.getAssetKeys()`][`sea.getAssetKeys()`].
...
Если `useSnapshot` равен `true`, основной скрипт должен вызвать [`v8.startupSnapshot.setDeserializeMainFunction()`][`v8.startupSnapshot.setDeserializeMainFunction()`], чтобы задать код, который выполнится при запуске итогового исполняемого файла пользователем.
...
На основной скрипт при сборке снимка распространяются общие ограничения сценариев startup snapshot; можно использовать [`v8.startupSnapshot` API][`v8.startupSnapshot` API]. См. [документацию по startup snapshot в Node.js][documentation about startup snapshot support in Node.js].
...
`require()` здесь не совпадает с [`require()`][`require()`] у обычных не встроенных модулей.
Сейчас у него нет свойств не встроенного [`require()`][`require()`], кроме [`require.main`][`require.main`].

### `__filename` и `module.filename` во встроенном основном скрипте

Значения `__filename` и `module.filename` равны [`process.execPath`][`process.execPath`].

### `__dirname` во встроенном основном скрипте

`__dirname` равен каталогу [`process.execPath`][`process.execPath`].

### `import.meta` во встроенном основном скрипте

При `"mainFormat": "module"` во встроенном скрипте доступен `import.meta` со свойствами:

* `import.meta.url`: `file:` URL, соответствующий [`process.execPath`][`process.execPath`].
* `import.meta.filename`: равен [`process.execPath`][`process.execPath`].
* `import.meta.dirname`: каталог [`process.execPath`][`process.execPath`].
