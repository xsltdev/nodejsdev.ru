...
Одноисполняемое приложение может запускать один встроенный скрипт в системе модулей [CommonJS][CommonJS] или [ECMAScript Modules][ECMAScript Modules].
...
На основной скрипт при сборке снимка распространяются общие ограничения сценариев startup snapshot; можно использовать [`v8.startupSnapshot` API](v8.md#startup-snapshot-api). См. [документацию по startup snapshot в Node.js][documentation about startup snapshot support in Node.js].
...
Аналогично [`sea.getAsset()`](#seagetassetkey-encoding), но результат — [Blob](https://developer.mozilla.org/en-US/docs/Web/API/Blob).
Если ресурс не найден, выбрасывается ошибка.
...
`require()` здесь не совпадает с [`require()`](modules.md#requireid) у обычных не встроенных модулей.
Сейчас у него нет свойств не встроенного [`require()`](modules.md#requireid), кроме [`require.main`](modules.md#accessing-the-main-module).
...
Записывается файл, который можно внедрить в бинарник Node.js инструментами вроде [postject][postject].
...
* Если бинарник `node` — [PE][PE], блоб внедряется как ресурс с именем `NODE_SEA_BLOB`.
* Если это [Mach-O][Mach-O], блоб внедряется как секция `NODE_SEA_BLOB` в сегменте `NODE_SEA`.
* Если это [ELF][ELF], блоб внедряется как нота `NODE_SEA_BLOB`.

Затем процесс сборки SEA ищет в бинарнике строку [fuse][fuse] `NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2:0` и переводит последний символ в `1`, чтобы отметить внедрение ресурса.

##### Ручное внедрение подготовительного блоба

До `--build-sea` использовался сценарий с внешними инструментами.

Например, с [postject][postject]:
...
2. Снимите подпись бинарника (только macOS и Windows):
   ...
   * На Windows (по желанию):
   ...
   [signtool][signtool] из [Windows SDK][Windows SDK]. Если шаг пропущен, игнорируйте предупреждения postject о подписи.
...
3. Внедрите блоб в скопированный бинарник через `postject` с опциями:
   ...
   * `--sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2` — [fuse][fuse], которым Node.js определяет внедрение.
...
* Windows
* macOS
* Linux (все дистрибутивы, [поддерживаемые Node.js][поддерживаемые Node.js], кроме Alpine, и все архитектуры, [поддерживаемые Node.js][поддерживаемые Node.js], кроме s390x)
...
[поддерживаемые Node.js]: https://github.com/nodejs/node/blob/main/BUILDING.md#platform-list
