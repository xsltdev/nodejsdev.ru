...
[:octicons-tag-24: последняя версия](https://nodejs.org/docs/latest/api/stream.html)
...
Событие `'error'` испускается при ошибке записи или передачи по каналу `pipe`. Обработчику
...
на паузе, пока не подключён канал `pipe` или обработчики `'data'`/`'readable'`.
...
`Readable` работает в одном из режимов: потоковом (`flowing`) или приостановленном (`paused`).
Это не то же самое, что [объектный режим][object-mode]: объектный режим может быть включён или нет независимо.
...
Из соображений совместимости удаление обработчиков [`'data'`](#event-data) **не** ставит поток на паузу автоматически.
...
Если `Readable` переведён в режим `flowing` и некому обрабатывать данные, они теряются (например
`readable.resume()` без слушателя `'data'` или снятый обработчик `'data'`).

Обработчик [`'readable'`](#event-readable) выводит поток из режима `flowing`; данные читают через
[`readable.read()`](#readablereadsize). Если `'readable'` снят, режим `flowing` возобновится при наличии
[`'data'`](#event-data).
...
* `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) для строки — кодировка символов; для `Buffer` или объектного режима
