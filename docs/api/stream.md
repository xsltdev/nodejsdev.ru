...
вместо этого нужно реализовать [`writable._destroy()`][writable-_destroy].
...
если файл меньше 64 КиБ, потому что [`fs.createReadStream()`][`fs.createReadStream()`] вызывается без `highWaterMark`.
...
протокол [`Stream.toAsyncStreamable`][`Stream.toAsyncStreamable`], что позволяет эффективно
потреблять их через API [`stream/iter`][`stream/iter`].
...
Возвращаемый итератор помечается как проверенный источник, поэтому [`from()`][stream-iter-from]
...
Вызывает [`readable.destroy()`][readable-destroy] с `AbortError` и возвращает
...
Подробнее см. [`stream.compose(...streams)`][`stream.compose(...streams)`].
...
сначала обработать фрагменты параллельно, используйте [`readable.map`][`readable.map`], а затем
`reduce`.
...
Потоки `Duplex` реализуют и [`Readable`][`Readable`], и [`Writable`][`Writable`].
...
Потоки `Transform` — это [`Duplex`][`Duplex`], выход которых тем или иным образом связан со входом.
Как и у любого [`Duplex`][`Duplex`], у `Transform` есть и [`Readable`][`Readable`], и [`Writable`][`Writable`].
...
* `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) значение, передаваемое в оба конструктора [`Duplex`][`Duplex`],
  например для настройки буферизации.
* Возвращает: [`<Array>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array) из двух экземпляров [`Duplex`][`Duplex`].
...
[`readable.compose(stream)`][`readable.compose(stream)`] как обёртка над этой функцией.
...
| --------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------ |
| Только чтение                                 | [`Readable`][`Readable`]  | [`_read()`][stream-_read]                                                                                          |
| Только запись                                 | [`Writable`][`Writable`]  | [`_write()`][stream-_write], [`_writev()`][stream-_writev], [`_final()`][stream-_final]                            |
| Чтение и запись                               | [`Duplex`][`Duplex`]    | [`_read()`][stream-_read], [`_write()`][stream-_write], [`_writev()`][stream-_writev], [`_final()`][stream-_final] |
| Обработка записанного и чтение результата     | [`Transform`][`Transform`] | [`_transform()`][stream-_transform], [`_flush()`][stream-_flush], [`_final()`][stream-_final]                      |
...
Класс `stream.Writable` расширяют, чтобы получить [`Writable`][`Writable`].
...
* `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  * `highWaterMark` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) уровень буфера, при котором
    [`stream.write()`][stream-write] начинает возвращать `false`. **По умолчанию:**
  ...
  * `decodeStrings` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) кодировать ли строки, переданные в
    [`stream.write()`][stream-write], в `Buffer` (с кодировкой из вызова
    [`stream.write()`][stream-write]) перед передачей в [`stream._write()`][stream-_write]. Прочие типы не
  ...
  * `defaultEncoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) кодировка по умолчанию, если в
    [`stream.write()`][stream-write] кодировка не указана.
    **По умолчанию:** `'utf8'`.
  * `objectMode` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) допустима ли операция
    [`stream.write(anyObj)`][stream-write]. При `true` можно писать произвольные значения JavaScript, не только
...
* `chunk` [`<Buffer>`](buffer.md#buffer) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | any `Buffer` для записи, полученный из строки, переданной в
  [`stream.write()`][stream-write]. Если у потока `decodeStrings` равен `false` или включён object mode,
...
[`writable._write()`][stream-_write] и/или
[`writable._writev()`][stream-_writev] для передачи данных нижележащему ресурсу.

[`Transform`][`Transform`] задаёт собственную реализацию
...
буферизуются. После `callback` поток может испустить [`'drain'`][`'drain'`]. Если реализация
...
`_destroy()` вызывается из [`writable.destroy()`][writable-destroy].
...
Ошибки в [`writable._write()`][`writable._write()`],
[`writable._writev()`][`writable._writev()`] и [`writable._final()`][`writable._final()`] нужно передавать в колбэк первым аргументом.
...
`Writable`; она показывает обязательные элементы [`Writable`][`Writable`]:
...
При многобайтовых кодировках (например UTF-8) это нетривиально. Ниже — пример с `StringDecoder` и [`Writable`][`Writable`].
...
Класс `stream.Readable` расширяют для реализации [`Readable`][`Readable`].

Пользовательский `Readable` _обязан_ вызывать `new stream.Readable([options])`
и реализовать [`readable._read()`][`readable._read()`].
...
* `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  ...
  * `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) если задано, буферы декодируются в строки в этой кодировке.
    **По умолчанию:** `null`.
  * `objectMode` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) поток объектов: [`stream.read(n)`][stream-read] возвращает
...
Любой `Readable` должен реализовать [`readable._read()`][`readable._read()`] для получения данных из ресурса.

Когда вызывается [`readable._read()`][`readable._read()`] и данные доступны, их следует помещать в очередь через
[`this.push(dataChunk)`][stream-push]. `_read()` вызовут снова после каждого [`this.push(dataChunk)`][stream-push],
...
После вызова [`readable._read()`][`readable._read()`] он не повторится, пока снова не будет данных через
[`readable.push()`][stream-push]. Пустые буферы и строки не приводят к повторному [`readable._read()`][`readable._read()`].
...
`_destroy()` вызывается из [`readable.destroy()`][readable-destroy].
...
В приостановленном режиме данные из `readable.push()` забирают через [`readable.read()`][stream-read] при [`'readable'`][`'readable'`].
...
`readable.push()` наполняет внутренний буфер; его обычно драйвит [`readable._read()`][`readable._read()`].

Вне object mode `chunk === undefined` трактуется как пустая строка или буфер. Подробнее — [`readable.push('')`][`readable.push('')`].

#### Ошибки при чтении

Ошибки при выполнении [`readable._read()`][`readable._read()`] передают через [`readable.destroy(err)`][readable-_destroy].
`throw` из [`readable._read()`][`readable._read()`] или ручной `emit('error')` дают неопределённое поведение.
...
[`Duplex`][`Duplex`] сочетает [`Readable`][`Readable`] и [`Writable`][`Writable`], как TCP-сокет.

В JavaScript нет множественного наследования, поэтому [`Duplex`][`Duplex`] получают расширением `stream.Duplex`, а не одновременным наследованием от `stream.Readable` и `stream.Writable`.

`stream.Duplex` прототипно наследует `stream.Readable` и подмешивает `stream.Writable`; `instanceof` для обоих базовых типов работает за счёт переопределения [`Symbol.hasInstance`][`Symbol.hasInstance`] у `stream.Writable`.

Пользовательский `Duplex` _обязан_ вызывать `new stream.Duplex([options])` и реализовать и [`readable._read()`][`readable._read()`], и `writable._write()`.
...
Второй пример — `Duplex`, который буферизует входящие данные через [`Writable`][`Writable`], а читает их через [`Readable`][`Readable`].
...
В примере ниже создаётся `Transform` (вид [`Duplex`][`Duplex`]): сторона записи в object mode принимает числа, на стороне чтения они превращаются в шестнадцатеричные строки.
...
[`Transform`][`Transform`] — это [`Duplex`][`Duplex`], выход которого как-то вычисляется из входа. Примеры: [zlib][zlib] и [crypto][crypto] для сжатия, шифрования и расшифровки.
...
Класс `stream.Transform` расширяют для [`Transform`][`Transform`].

`stream.Transform` наследует `stream.Duplex` и подставляет свои `writable._write()` и
[`readable._read()`][`readable._read()`]. Пользовательская реализация _обязана_ задать
[`transform._transform()`][stream-_transform] и _может_ задать [`transform._flush()`][stream-_flush].
...
* `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) передаётся в конструкторы `Writable` и `Readable`, плюс поля:
  * `transform` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) реализация [`stream._transform()`][stream-_transform].
  * `flush` [`<Function>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) реализация [`stream._flush()`][stream-_flush].
...
Событие [`'end'`][`'end'`] относится к `stream.Readable`. Оно испускается после вывода всех данных,
то есть после колбэка [`transform._flush()`][stream-_flush]. При ошибке `'end'` не должен испускаться.

#### Событие: `'finish'`

Событие [`'finish'`][`'finish'`] относится к `stream.Writable`. Оно испускается после [`stream.end()`][stream-end],
когда все фрагменты обработаны [`stream._transform()`][stream-_transform]. При ошибке `'finish'` не должен испускаться.
...
Реализации [`Transform`][`Transform`] _могут_ определить `transform._flush()`: вызывается, когда входящих данных больше нет, но до [`'end'`][`'end'`] на [`Readable`][`Readable`].
...
* `chunk` [`<Buffer>`](buffer.md#buffer) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | any `Buffer` для преобразования из строки, переданной в [`stream.write()`][stream-write]. При `decodeStrings: false` или object mode фрагмент не преобразуется.
...
`stream.PassThrough` — тривиальная реализация [`Transform`][`Transform`], пробрасывающая байты с входа на выход. Чаще всего для примеров и тестов, но иногда служит кирпичиком для нестандартных потоков.
...
При записи из async-итератора важно учитывать обратное давление и ошибки. [`stream.pipeline()`][`stream.pipeline()`] скрывает эту логику:
...
* Вместо ожидания [`stream.read()`][stream-read] события [`'data'`][`'data'`] шли сразу. Если обработка требовала работы, данные приходилось буферизовать, чтобы не потерять.
* [`stream.pause()`][stream-pause] был лишь рекомендацией: [`'data'`][`'data'`] всё равно могли приходить _даже в «приостановленном» состоянии_.

В Node.js 0.10 добавили класс [`Readable`][`Readable`]. Для совместимости со старым кодом поток переходит в «потоковый режим» при подписке на [`'data'`][`'data'`] или вызове [`stream.resume()`][stream-resume]. Поэтому даже без [`stream.read()`][stream-read] и [`'readable'`][`'readable'`] фрагменты [`'data'`][`'data'`] обычно не теряются.

В большинстве случаев всё работает, но есть пограничный случай:

* нет подписчика на [`'data'`][`'data'`];
* не вызывается [`stream.resume()`][stream-resume];
...
Обходной путь — вызвать [`stream.resume()`][stream-resume], чтобы запустить поток данных:
...
Кроме того, старые потоки до 0.10 можно обернуть в класс `Readable` через [`readable.wrap()`][`stream.wrap()`].

### `readable.read(0)`

Иногда нужно «подтолкнуть» механизм чтения, не потребляя данные — тогда вызывают `readable.read(0)`; результат всегда `null`.

Если внутренний буфер ниже `highWaterMark` и поток сейчас не читает, `stream.read(0)` инициирует низкоуровневый вызов [`stream._read()`][stream-_read].
...
Пустая [string](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type), [Buffer](buffer.md#buffer), [TypedArray](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) или [DataView](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) вне object mode даёт особый эффект: это всё равно вызов [`readable.push()`][stream-push], завершающий чтение, но в буфер не попадает данных — потребителю нечего читать.
