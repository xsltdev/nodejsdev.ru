...
[запрос к HTTP-серверу][http-incoming-message] и [`process.stdout`][]
...
* [`Writable`][]: потоки, в которые можно записывать данные (например,
  [`fs.createWriteStream()`][]).
* [`Readable`][]: потоки, из которых можно читать данные (например,
  [`fs.createReadStream()`][]).
* [`Duplex`][]: потоки, которые являются одновременно `Readable` и `Writable` (например,
  [`net.Socket`][]).
...
И [`Writable`][], и [`Readable`][] потоки хранят данные во внутреннем буфере.
...
перестаёт вызываться внутренний метод [`readable._read()`][], которым
...
Ключевая цель API `stream`, в частности метода [`stream.pipe()`][],
...
Поскольку потоки [`Duplex`][] и [`Transform`][] одновременно `Readable` и
...
эффективный поток данных. Например, экземпляры [`net.Socket`][] — это [`Duplex`][], у которых сторона `Readable` позволяет
...
Потоки [`Writable`][] (например `res` в примере) предоставляют методы вроде
`write()` и `end()` для записи данных в поток.

Потоки [`Readable`][] используют API [`EventEmitter`][], чтобы уведомлять код
о появлении данных; их можно читать разными способами.

И [`Writable`][], и [`Readable`][] используют [`EventEmitter`][] для передачи
состояния потока.

Потоки [`Duplex`][] и [`Transform`][] одновременно [`Writable`][] и
[`Readable`][].
...
Некоторые из них на самом деле [`Duplex`][], реализующие интерфейс [`Writable`][].

Все потоки [`Writable`][] реализуют интерфейс класса `stream.Writable`.

Конкретные экземпляры [`Writable`][] могут отличаться, но общий шаблон
...
Поток [`Writable`][] всегда испускает `'close'`, если создан с опцией `emitClose`.
...
Событие `'pipe'` испускается при вызове [`stream.pipe()`][] на читаемом потоке,
...
Событие `'unpipe'` испускается при вызове [`stream.unpipe()`][] на [`Readable`][],
когда этот [`Writable`][] удаляется из назначений.

Также испускается, если этот [`Writable`][] выдаёт ошибку при подключении [`Readable`][] по pipe.
...
происходит при вызове [`stream.uncork()`][] или [`stream.end()`][stream-end].
...
См. также: [`writable.uncork()`][], [`writable._writev()`][stream-_writev].
...
`writable.setDefaultEncoding()` задаёт кодировку по умолчанию для [`Writable`][].
...
`writable.uncork()` сбрасывает данные, буферизованные после [`stream.cork()`][].

При использовании [`writable.cork()`][]/`writable.uncork()` откладывайте `uncork`
...
Если [`writable.cork()`][] вызывали несколько раз, столько же раз нужно вызвать
...
`true` после вызова [`writable.end()`][]. Не отражает, сброшены ли данные; для этого
см. [`writable.writableFinished`][].
...
Становится `true` непосредственно перед событием [`'finish'`][].
...
* Возвращает: [<boolean>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) `false`, если нужно дождаться [`'drain'`][] перед дальнейшей записью; иначе `true`.
...
При `false` не продолжайте запись до [`'drain'`][].
...
Для [`Transform`][] запись при отсутствии drain особенно критична: `Transform` по умолчанию
на паузе, пока не подключён pipe или обработчики `'data'`/`'readable'`.

Если данные можно получать по требованию, лучше оформить это как [`Readable`][] и
[`stream.pipe()`][]. Если нужен именно `write()`, учитывайте обратное давление через
[`'drain'`][]:
...
Все потоки [`Readable`][] реализуют интерфейс класса `stream.Readable`.
...
* В потоковом режиме данные читаются из нижележащей системы автоматически и по возможности быстро
  доставляются приложению через события [`EventEmitter`][].

* В приостановленном режиме порции нужно явно забирать через [`stream.read()`][stream-read].

Изначально все [`Readable`][] в paused, переход в flowing:

* подписка на [`'data'`][];
* вызов [`stream.resume()`][stream-resume];
* вызов [`stream.pipe()`][] к [`Writable`][].

Обратно в paused:

* без назначений pipe — [`stream.pause()`][stream-pause];
* с pipe — удалить все назначения (в том числе через [`stream.unpipe()`][]).
...
Из совместимости: удаление обработчиков [`'data'`][] **не** ставит поток на паузу автоматически.
...
Обработчик [`'readable'`][] переводит поток из flowing; данные читают через
...
Поток [`Readable`][] всегда испускает `'close'`, если создан с опцией `emitClose`.
...
Если одновременно используются `'readable'` и [`'data'`][], потоком управляет `'readable'`:
...
`readable.pause()` останавливает испускание [`'data'`][] в потоковом режиме
...
* Возвращает: [<stream.Writable>](stream.md#streamwritable) _destination_ — для цепочки pipe, если это [`Duplex`][] или [`Transform`][]

`readable.pipe()` подключает [`Writable`][] к `readable`, переводит его в потоковый режим
и передаёт все данные в этот [`Writable`][]. Поток данных регулируется так, чтобы быстрый
...
`Readable` испускает [`'end'`][], и запись завершается. Чтобы оставить поток открытым,
передайте `end: false`:
...
[`process.stderr`][] и [`process.stdout`][] не закрываются до выхода процесса Node.js, независимо от опций.
...
После [`'end'`][] вызов [`stream.read([size])`][stream-read] даёт `null` без исключения.
...
Геттер свойства `encoding` для данного `Readable`. Кодировку задаёт [`readable.setEncoding()`][].
...
Становится `true` при событии [`'end'`][].
...
Отражает состояние `Readable`, см. раздел [Три состояния][].
...
`readable.resume()` возобновляет испускание [`'data'`][] у явно приостановленного `Readable`,
...
`readable.unpipe()` отсоединяет ранее подключённый через [`stream.pipe()`][] `Writable`.
...
После [`'end'`][] вызвать `stream.unshift(chunk)` нельзя — будет ошибка.

Часто уместнее [`Transform`][] вместо `stream.unshift()`; см. [API для реализаторов потоков][API for stream implementers].
...
Если старая библиотека испускает [`'data'`][] и [`stream.pause()`][stream-pause] лишь рекомендательный,
`readable.wrap()` оборачивает её в новый [`Readable`][].
...
В отличие от подписки на [`'data'`][], опирается на механизм [`readable`][] и может ограничивать число параллельных вызовов `fn`.
...
* `options` [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) A value to pass to both [`Duplex`][] constructors,
  to set options such as buffering.
* Returns: [<Array>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array) of two [`Duplex`][] instances.
