...
источник не читается быстрее, чем успевает обработка. Потокам `push`
...
Потоки `push` используют двухчастную буферизацию. Представьте ведро
(слоты), заполняемое через шланг (ожидающие записи), с поплавковым клапаном,
