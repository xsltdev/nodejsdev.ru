...
Синхронный вариант [`from()`][]. Возвращает синхронный итерируемый объект. Не принимает
...
Синхронный вариант [`pipeTo()`][]. Источник, все преобразования и
...
* Возвращает: [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  * `writer` {PushWriter} Сторона writer.
...
Создаёт {Broadcast} из существующего источника. Источник потребляется
...
`pipeTo()` и `pipeToSync()` доступны в модуле [`node:zlib/iter`][].
...
[`stream.Readable`][]/[`stream.Writable`][] с API `stream/iter`.
...
источник async iterable stream/iter, который можно передать в [`from()`][],
[`pull()`][], [`text()`][] и т.д.

Если объект реализует протокол [`toAsyncStreamable`][] (как
...
Создаёт адаптер Writer stream/iter из классического Writable (или эквивалента по контракту). Его можно передать в [`pipeTo()`][] как
назначение.
...
Создаёт [`stream.Readable`][] в байтовом режиме из `AsyncIterable<Uint8Array[]>`
...
* `source` [<Iterable>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Iteration_protocols#The_iterable_protocol) Источник `Iterable<Uint8Array[]>`, например
  результат [`pullSync()`][] или [`fromSync()`][].
* `options` [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  * `highWaterMark` [<number>](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Внутренний размер буфера в байтах до включения
    обратного давления. **По умолчанию:** `65536` (64 КБ).
* Возвращает: [<stream.Readable>](stream.md#streamreadable)

Создаёт [`stream.Readable`][] в байтовом режиме из синхронного
...
интерфейсу {Broadcast}. Реализация полностью на усмотрение автора — можно
...
интерфейсу {Share}. Реализация полностью на усмотрение автора — общий
...
интерфейсу {SyncShare}. Реализация полностью на усмотрение автора — общий
