...
-   `node.async_hooks`: включает захват подробных данных трассировки [`async_hooks`](async_hooks.md). События [`async_hooks`](async_hooks.md) имеют уникальный `asyncId` и особое свойство `triggerAsyncId` (в тексте также упоминается `triggerId`).
...
-   `v8`: события [V8](v8.md) связаны со сборкой мусора, компиляцией и выполнением.
...
=== "MJS"
    ...
    tracing.enable();  // включить сбор для категории 'node.perf'

    // работа приложения

    tracing.disable();  // отключить сбор для категории 'node.perf'
    ```
...
Возможности этого модуля недоступны в потоках [`Worker`](worker_threads.md#class-worker).
...
=== "MJS"
    ...
    // Выведет 'node,node.perf,v8'
    console.log(getEnabledCategories());

    t2.disable(); // Отключает только категорию 'node.perf'
...
-   Возвращает: [`<Tracing>`](#tracing-object).
...
### Сбор данных событий трассировки через модуль inspector
