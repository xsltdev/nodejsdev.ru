...
* `node.async_hooks`: включает захват подробных данных трассировки [`async_hooks`][]. События [`async_hooks`][] имеют уникальный `asyncId` и особое свойство `triggerAsyncId` (в тексте также упоминается `triggerId`).
...
* `v8`: события [V8][] связаны со сборкой мусора, компиляцией и выполнением.
...
Возможности этого модуля недоступны в потоках [`Worker`][].
