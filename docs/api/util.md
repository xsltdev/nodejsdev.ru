...
исключение, процесс испустит [`'uncaughtException'`](process.md#event-uncaughtexception), и при отсутствии
...
[`console.error()`](console.md#consoleerrordata-args)); иначе функция ничего не делает.
...
[`util.diff()`](#utildiffactual-expected) сравнивает две строки или массива и возвращает массив записей различий.
...
Как [`util.format()`](#utilformatformat-args), но дополнительно принимает `inspectOptions` — опции для
...
* `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
  ...
  * `colors` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, вывод оформляется ANSI-кодами цвета.
    Цвета настраиваются. См. [настройку цветов `util.inspect`](#customizing-utilinspect-colors).
    **По умолчанию:** `false`.
  ...
  * `showProxy` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Если `true`, при осмотре `Proxy` включаются
    объекты [`target` и `handler`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy#terminology). **По умолчанию:** `false`.
...
Чтобы проверить поддержку цветов, используйте [`tty.hasColors()`](tty.md#writestreamhascolorscount-env).
...
объект с одним или несколькими допустимыми параметрами [`util.inspect()`](#utilinspectobject-options).
...
Возвращает объект [`MIMEParams`](#class-utilmimeparams), представляющий
...
через [`JSON.stringify()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify).
...
См. также [`util.types.isArrayBuffer()`](#utiltypesisarraybuffervalue) и
...
Не включает экземпляры [SharedArrayBuffer](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer). Обычно нужно проверять оба; см. [`util.types.isAnyArrayBuffer()`](#utiltypesisanyarraybuffervalue).
...
В JavaScript это [замороженные](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/freeze) объекты с прототипом `null`.
...
Не включает экземпляры [ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). Обычно нужно проверять оба; см. [`util.types.isAnyArrayBuffer()`](#utiltypesisanyarraybuffervalue).
