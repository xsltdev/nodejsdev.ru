...
Модуль `node:v8` предоставляет API, специфичные для версии [V8][], встроенной в
...
у [`vm.Script`][] с текущим экземпляром V8.
...
[`GetHeapCodeAndMetadataStatistics`][]. Объект со свойствами:
...
пространств и их набор не гарантируются: данные приходят из [`GetHeapSpaceStatistics`][]
...
Возвращает статистику [CppHeap][] по памяти через V8 [`CollectStatistics()`][]; формат
...
Структура близка к [`cppgc::HeapStatistics`][]; подробности — в [документации V8][`cppgc::HeapStatistics struct`].
...
Аналог [`queryObjects()` console API][] в консоли Chromium DevTools. После полного GC
...
`v8.stopCoverage()` останавливает сбор покрытия, запущенный [`NODE_V8_COVERAGE`][], чтобы V8
освободил счётчики и мог оптимизировать код. Сочетается с [`v8.takeCoverage()`][] при
...
`v8.takeCoverage()` записывает на диск покрытие, начатое [`NODE_V8_COVERAGE`][], по запросу.
Метод можно вызывать многократно: каждый раз счётчик сбрасывается и пишется новый отчёт
в каталог из [`NODE_V8_COVERAGE`][].
...
Снимок относится к одному isolate. При [worker threads][] снимок с главного потока не
...
повторно. `limit` — положительное целое. Подробнее: [`--heapsnapshot-near-heap-limit`][].
...
[`ERR_BUFFER_TOO_LARGE`][], если объект слишком велик для буфера больше
...
* `buffer` [<Buffer>](buffer.md#buffer) | [<TypedArray>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [<DataView>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) Буфер из [`serialize()`][].
...
`ArrayBuffer` в [`deserializer.transferArrayBuffer()`][].
...
Для [`serializer._writeHostObject()`][].
...
* `buffer` [<Buffer>](buffer.md#buffer) | [<TypedArray>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [<DataView>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) Буфер из [`serializer.releaseBuffer()`][].
...
[`serializer.transferArrayBuffer()`][] или `id` из [`serializer._getSharedArrayBufferId()`][] для `SharedArrayBuffer`.
...
Сырые байты; `length` должен совпадать с [`serializer.writeRawBytes()`][].
Для [`deserializer._readHostObject()`][].
...
Подкласс [`Serializer`][]: сериализует `TypedArray` (в т.ч. [`Buffer`][]) и `DataView`
...
активности см. [`async_hooks`][] (внутри использует этот модуль). Для контекста запросов
см. [`AsyncLocalStorage`][].
...
* `init` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Колбэк [`init` callback][] при создании промиса.
...
* `settled` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Колбэк [`settled` callback][] при выполнении или отклонении.
* Возвращает: [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Функция для отключения хука.
...
* `before` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Колбэк [`before` callback][] до выполнения продолжения промиса.
* Возвращает: [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Функция для отключения хука.
...
* `after` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Колбэк [`after` callback][] после выполнения продолжения промиса.
* Возвращает: [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) Функция для отключения хука.
...
* `callbacks` [<Object>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) [Hook Callbacks][] для регистрации
  * `init` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) [`init` callback][].
  * `before` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) [`before` callback][].
  * `after` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) [`after` callback][].
  * `settled` [<Function>](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function) [`settled` callback][].
...
Подробности — в разделе [Hook Callbacks][].
...
В отличие от [`async_hooks`][] здесь нет `destroy`: у сокетов есть «закрыто», у промисов —
