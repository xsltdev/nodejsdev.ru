...
Сжатие и распаковка опираются на [Streams API](stream.md) Node.js.
...
Модуль `node:zlib` позволяет поддержать механизмы `gzip`, `deflate`, `br` и `zstd` для `content-encoding` из [HTTP](https://tools.ietf.org/html/rfc7230#section-4.2).

Заголовок [`Accept-Encoding`](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.3) в запросе указывает, какие кодировки сжатия принимает клиент. Заголовок [`Content-Encoding`](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.11) — какие кодировки сжатия фактически применены к сообщению.

Примеры ниже сильно упрощены. Кодирование через `zlib` может быть дорогим, результаты стоит кэшировать. См. [настройку использования памяти](#memory-usage-tuning) о компромиссе скорость/память/степень сжатия для `zlib`.
...
Подробнее о параметрах Brotli — [ниже](#brotli-constants).
...
Подробнее о параметрах Zstd — [ниже](#zstd-constants).

## Сброс (flush)

Вызов [`.flush()`](#zlibflushkind-callback) на потоке сжатия заставляет `zlib` отдать максимум выходных данных на текущий момент. Качество сжатия может ухудшиться, зато данные быстрее становятся доступны.
...
Все константы из `zlib.h` также доступны в `require('node:zlib').constants`. В обычной работе они часто не нужны; они описаны, чтобы наличие не удивляло. Раздел почти взят из [документации zlib](https://zlib.net/manual.html#Constants).
...
-   `BROTLI_PARAM_LARGE_WINDOW`
    -   Boolean: режим «Large Window Brotli» (несовместим с форматом Brotli в [RFC 7932](https://www.rfc-editor.org/rfc/rfc7932.txt)).
...
-   `BROTLI_DECODER_PARAM_LARGE_WINDOW`
    -   Boolean: режим «Large Window Brotli» (несовместим с [RFC 7932](https://www.rfc-editor.org/rfc/rfc7932.txt)).

### Константы Zstd {#zstd-constants}
...
-   `dictionary` [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) | [`<ArrayBuffer>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) (только deflate/inflate, по умолчанию пустой словарь)
...
-   `maxOutputLength` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Ограничивает размер выхода у [вспомогательных методов](#convenience-methods). **По умолчанию:** [`buffer.kMaxLength`](buffer.md#bufferkmaxlength)
...
-   `chunkSize` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `16 * 1024`
-   `params` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) Объект «ключ — значение» с индексированными [параметрами Brotli](#brotli-constants).
-   `maxOutputLength` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Ограничивает размер выхода у [вспомогательных методов](#convenience-methods). **По умолчанию:** [`buffer.kMaxLength`](buffer.md#bufferkmaxlength)
...
Наследует [`stream.Transform`](stream.md#class-streamtransform), поэтому объекты `node:zlib` можно использовать в цепочках `pipe` и подобных операциях с потоками.
...
-   `chunkSize` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `16 * 1024`
-   `params` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) Объект «ключ — значение» с индексированными [параметрами Zstd](#zstd-constants).
-   `maxOutputLength` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Ограничивает размер выхода у [вспомогательных методов](#convenience-methods). **По умолчанию:** [`buffer.kMaxLength`](buffer.md#bufferkmaxlength)
-   `info` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) При `true` возвращается объект с `buffer` и `engine`. **По умолчанию:** `false`
-   `dictionary` [`<Buffer>`](buffer.md#buffer) Необязательный словарь для повышения эффективности сжатия/распаковки данных с общими с словарём шаблонами.
...
## Класс: `zlib.ZstdCompress` {#class-zlibzstdcompress}
...
## Класс: `zlib.ZstdDecompress` {#class-zlibzstddecompress}
...
Вычисляет 32-битную контрольную сумму [CRC](https://en.wikipedia.org/wiki/Cyclic_redundancy_check) для `data`. Если задан `value`, оно используется как начальное значение суммы, иначе начинаем с 0.
...
-   `options` [`<brotli options>`](zlib.md#class-brotlioptions)

Создаёт и возвращает новый объект [`BrotliCompress`](#class-zlibbrotlicompress).

## `zlib.createBrotliDecompress([options])`

-   `options` [`<brotli options>`](zlib.md#class-brotlioptions)

Создаёт и возвращает новый объект [`BrotliDecompress`](#class-zlibbrotlidecompress).

## `zlib.createDeflate([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`Deflate`](#class-zlibdeflate).

## `zlib.createDeflateRaw([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`DeflateRaw`](#class-zlibdeflateraw).
...
Создаёт и возвращает новый объект [`Gunzip`](#class-zlibgunzip).

## `zlib.createGzip([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`Gzip`](#class-zlibgzip). См. [пример](#zlib).

## `zlib.createInflate([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`Inflate`](#class-zlibinflate).

## `zlib.createInflateRaw([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`InflateRaw`](#class-zlibinflateraw).

## `zlib.createUnzip([options])`

-   `options` [`<zlib options>`](zlib.md#class-options)

Создаёт и возвращает новый объект [`Unzip`](#class-zlibunzip).
...
-   `options` [`<zstd options>`](zlib.md#class-zstdoptions)

Создаёт и возвращает новый объект [`ZstdCompress`](#class-zlibzstdcompress).
...
Создаёт и возвращает новый объект [`ZstdDecompress`](#class-zlibzstddecompress).

## Вспомогательные методы {#convenience-methods}

Все эти методы принимают первым аргументом [Buffer](buffer.md#buffer), [TypedArray](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray), [DataView](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView), [ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) или строку, вторым (необязательно) — опции для классов `zlib`, и вызывают переданный callback в виде `callback(error, result)`.
...
Сжимает фрагмент данных через [`BrotliCompress`](#class-zlibbrotlicompress).
...
Распаковывает фрагмент данных через [`BrotliDecompress`](#class-zlibbrotlidecompress).
...
Сжимает фрагмент данных через [`Deflate`](#class-zlibdeflate).
...
Сжимает фрагмент данных через [`DeflateRaw`](#class-zlibdeflateraw).
...
Распаковывает фрагмент данных через [`Gunzip`](#class-zlibgunzip).
...
Сжимает фрагмент данных через [`Gzip`](#class-zlibgzip).
...
Распаковывает фрагмент данных через [`Inflate`](#class-zlibinflate).
...
Распаковывает фрагмент данных через [`InflateRaw`](#class-zlibinflateraw).
...
Распаковывает фрагмент данных через [`Unzip`](#class-zlibunzip).
...
Сжимает фрагмент данных через [`ZstdCompress`](#class-zlibzstdcompress).
...
Распаковывает фрагмент данных через [`ZstdDecompress`](#class-zlibzstddecompress).
