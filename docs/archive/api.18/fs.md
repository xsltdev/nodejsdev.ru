...
## Пример обещания {: #promise-example}
...
Объект [`<FileHandle>`](#class-filehandle) является объектной оберткой для числового дескриптора файла.

Экземпляры объекта [`<FileHandle>`](#class-filehandle) создаются методом `fsPromises.open()`.

Все объекты [`<FileHandle>`](#class-filehandle) являются [`<EventEmitter>`](events.md#class-eventemitter).

Если [`<FileHandle>`](#class-filehandle) не закрыт с помощью метода `filehandle.close()`, он попытается автоматически закрыть дескриптор файла и выдать предупреждение процессу, помогая предотвратить утечку памяти. Пожалуйста, не полагайтесь на это поведение, поскольку оно может быть ненадежным, и файл может быть не закрыт. Вместо этого всегда явно закрывайте [`<FileHandle>`](#class-filehandle)s. Node.js может изменить это поведение в будущем.

#### Событие: `'close'`

Событие `'close'` происходит, когда [`<FileHandle>`](#class-filehandle) был закрыт и больше не может быть использован.
...
-   [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Числовой дескриптор файла, управляемый объектом [`<FileHandle>`](#class-filehandle).
...
Файл [`<FileHandle>`](#class-filehandle) должен поддерживать чтение.
...
Изменяет временные метки файловой системы объекта, на который ссылается [`<FileHandle>`](#class-filehandle), затем разрешает обещание без аргументов в случае успеха.
...
Объект [`<FileHandle>`](#class-filehandle) должен поддерживать запись.
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](fs.md#class-filehandle) имя файла или [`<FileHandle>`](#class-filehandle)
...
Путь может быть указан как [`<FileHandle>`](#class-filehandle), который был открыт для добавления (с помощью `fsPromises.open()`).
...
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) Выполняется с объектом [`<FileHandle>`](#class-filehandle).
...
-   Возвращает: [`<Promise>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) Выполняется с помощью [`fs.Dir`](#class-fsdir).

Асинхронно открывает каталог для итеративного сканирования. Более подробно см. документацию POSIX opendir(3).

Создает [`fs.Dir`](#class-fsdir), который содержит все дальнейшие функции для чтения из каталога и его очистки.
...
При использовании асинхронного итератора объект [`fs.Dir`](#class-fsdir) будет автоматически закрыт после выхода итератора.
...
-   `path` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](fs.md#class-filehandle) имя файла или `FileHandle`
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<null>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Null_type) **По умолчанию:** `null`
...
-   `file` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<URL>`](url.md#the-whatwg-url-api) | [`<FileHandle>`](fs.md#class-filehandle) имя файла или `FileHandle`
-   `data` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Buffer>`](buffer.md#buffer) | [`<TypedArray>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) | [`<DataView>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/DataView) | [`<AsyncIterable>`](https://tc39.github.io/ecma262/#sec-asynciterable-interface) | [`<Iterable>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Iteration_protocols#The_iterable_protocol) | [`<Stream>`](stream.md#stream)
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) | [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type)
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<null>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Null_type) **По умолчанию:** `'utf8''
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
...
### fs.cp {#fscpsrc-dest-options-callback}
...
-   `options` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) | [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `encoding` [`<string>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#String_type) **По умолчанию:** `null`.
    -   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) | [`<FileHandle>`](fs.md#class-filehandle) **По умолчанию:** `null`.
    -   `mode` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) **По умолчанию:** `0o666`
    -   `autoClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `true`
    -   `emitClose` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) **По умолчанию:** `true`
    -   `start` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type)
...
Если указано `fd`, `ReadStream` будет игнорировать аргумент `path` и будет использовать указанный дескриптор файла. Это означает, что событие `'open'` не будет выдано. `fd` должен быть блокирующим; неблокирующие `fd` должны быть переданы в [`<net.Socket>`](net.md#class-netsocket).
...
Как и [`<fs.ReadStream>`](fs.md#class-fsreadstream), если указан `fd`, [`<fs.WriteStream>`](fs.md#class-fswritestream) будет игнорировать аргумент `path` и будет использовать указанный дескриптор файла. Это означает, что событие `'open'` не будет выдано. `fd` должен быть блокирующим; неблокирующие`fd` должны быть переданы в [`<net.Socket>`](net.md#class-netsocket).
...
### fs.symlinkSync {#fssymlinksynctarget-path-type}
...
Пример см. в разделе [`fs.Dir`](#class-fsdir).
...
Представление записи каталога, которая может быть файлом или подкаталогом в каталоге, возвращаемое при чтении из [`fs.Dir`](#class-fsdir). Запись каталога представляет собой комбинацию пар имени файла и типа файла.

Кроме того, когда вызывается [`fs.readdir()`](#fsreaddirpath-options-callback) или [`fs.readdirSync()`](#fsreaddirsync) с опцией `withFileTypes`, установленной в `true`, результирующий массив заполняется объектами [`<fs.Dirent>`](fs.md#fsdirent), а не строками или [`<Buffer>`](buffer.md#buffer).
...
Имя файла, на который ссылается данный объект [`<fs.Dirent>`](fs.md#fsdirent). Тип этого значения определяется `options.encoding`, переданным в [`fs.readdir()`](#fsreaddirpath-options-callback) или [`fs.readdirSync()`](#fsreaddirsync).
...
### fs.ReadStream {#class-fsreadstream}

-   Расширяет: [`<stream.Readable>`](stream.md#streamreadable)

Экземпляры [`<fs.ReadStream>`](fs.md#class-fsreadstream) создаются и возвращаются с помощью функции [`fs.createReadStream()`](#fscreatereadstreampath-options).

#### Событие: `close`

Выдается, когда базовый файловый дескриптор [`<fs.ReadStream>`](fs.md#class-fsreadstream) был закрыт.
...
Выдается, когда дескриптор файла [`<fs.ReadStream>`](fs.md#class-fsreadstream) был открыт.

#### Событие: `ready`

Вызывается, когда [`<fs.ReadStream>`](fs.md#class-fsreadstream) готов к использованию.
...
### fs.WriteStream {#class-fswritestream}

-   Расширяет [`<stream.Writable>`](stream.md#streamwritable)

Экземпляры [`<fs.WriteStream>`](fs.md#class-fswritestream) создаются и возвращаются с помощью функции [`fs.createWriteStream()`](#fscreatewritestreampath-options).

#### Событие: `close`

Выдается, когда базовый файловый дескриптор [`<fs.WriteStream>`](fs.md#class-fswritestream) был закрыт.

#### Событие: `open`

-   `fd` [`<integer>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Целочисленный дескриптор файла, используемый [`<fs.WriteStream>`](fs.md#class-fswritestream).

Выдается при открытии файла [`<fs.WriteStream>`](fs.md#class-fswritestream).

#### Событие: `ready`

Вызывается, когда [`<fs.WriteStream>`](fs.md#class-fswritestream) готов к использованию.
...
Путь к файлу, в который записывается поток, указанный в первом аргументе [`fs.createWriteStream()`](#fscreatewritestreampath-options). Если `path` передан как строка, то `writeStream.path` будет строкой. Если `path` передан как [`<Buffer>`](buffer.md#buffer), то `writeStream.path` будет [`<Buffer>`](buffer.md#buffer).
...
API, основанные на обещаниях, используют объект [`<FileHandle>`](#class-filehandle) вместо числового дескриптора файла. Эти объекты лучше управляются системой, чтобы исключить утечку ресурсов. Однако, по-прежнему требуется, чтобы они закрывались по завершении операций:
