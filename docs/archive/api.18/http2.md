...
-   `options` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)
    ...
    -   `maxHeaderListPairs` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Устанавливает максимальное количество записей заголовков. Аналогично [`server.maxHeadersCount`](http.md#servermaxheaderscount) или [`request.maxHeadersCount`](http.md#requestmaxheaderscount) в модуле `node:http`. Минимальное значение — `1`. **Default:** `128`.
    -   `maxOutstandingPings` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Устанавливает максимальное количество ожидающих неподтверждённых пингов. **Default:** `10`.
    -   `maxReservedRemoteStreams` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Устанавливает максимальное количество зарезервированных push-потоков, которые клиент примет в любой момент времени. Когда текущее количество зарезервированных push-потоков превышает этот лимит, новые push-потоки от сервера будут автоматически отклоняться. Минимальное значение — 0. Максимальное значение — 2<sup>32</sup>-1. Отрицательное значение устанавливает максимальное значение. **Default:** `200`.
    -   `maxSendHeaderBlockLength` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Устанавливает максимально допустимый размер сериализованного сжатого блока заголовков. Попытки отправить заголовки, превышающие этот лимит, приведут к событию `'frameError'` и закрытию/уничтожению потока.
    -   `paddingStrategy` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Стратегия, используемая для определения количества вставки для фреймов `HEADERS` и `DATA`. **По умолчанию:** `http2.constants.PADDING_STRATEGY_NONE`. Значение может быть одним из:
        -   `http2.constants.PADDING_STRATEGY_NONE`: Никакая прокладка не применяется.
        -   `http2.constants.PADDING_STRATEGY_MAX`: Применяется максимальное количество прокладок, определяемое внутренней реализацией.
...
-   `headerTableSize` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Определяет максимальное количество байтов, используемых для сжатия заголовков. Минимальное значение — 0. Максимальное значение — 2<sup>32</sup>-1. **Default:** `4096`.
...
-   `initialWindowSize` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Определяет начальный размер окна _отправителя_ в байтах для управления потоком на уровне потока. Минимальное значение — 0. Максимальное значение — 2<sup>32</sup>-1. **По умолчанию:** `65535`.
-   `maxFrameSize` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Определяет размер в байтах наибольшей полезной нагрузки фрейма. Минимальное значение — 16 384. Максимальное значение — 2<sup>24</sup>-1. **Default:** `16384`.
-   `maxConcurrentStreams` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Определяет максимальное количество одновременных потоков, разрешённых на `Http2Session`. Нет значения по умолчанию, что означает, что теоретически 2<sup>32</sup>-1 потоков могут быть открыты одновременно в `Http2Session`. Минимальное значение — 0. Максимальное значение — 2<sup>32</sup>-1. **Default:** `4294967295`.
-   `maxHeaderListSize` [`<number>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Number_type) Определяет максимальный размер (несжатые октеты) списка заголовков, который будет принят. Минимальное значение — 0. Максимальное значение — 2<sup>32</sup>-1. **Default:** `65535`.
