...
## Точки входа в пакет {#package-entry-points}
...
### Подпункт импорта {#subpath-imports}
...
### Условный экспорт {#conditional-exports}
...
### Самостоятельная ссылка на пакет с помощью его имени {#self-referencing-a-package-using-its-name}
...
## Двойные пакеты модулей CommonJS/ES {#dual-commonjses-module-packages}
...
## Определения полей Node.js `package.json` {#nodejs-packagejson-field-definitions}
