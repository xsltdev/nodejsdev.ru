...
Хотя CPU в основном простаивает во время выполнения этого сценария, значение `utilization` равно `1`. Это происходит потому, что вызов [`child_process.spawnSync()`](child_process.md#child_processspawnsynccommand-args-options) блокирует выполнение цикла событий.
