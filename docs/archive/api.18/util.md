...
Поддержка модификаторов различается в разных терминалах. В большинстве случаев они игнорируются, если не поддерживаются.

-   `reset` - Сбрасывает все модификаторы (цвета) в значения по умолчанию
-   **bold** - Делает текст жирным
-   _italic_ - Делает текст курсивом
-   <span style="border-bottom: 1px;">underline</span> - Делает текст подчёркнутым
...
-   <span style="opacity: 0.5;">dim</span> - Уменьшенная интенсивность цвета (псевдоним: `faint`)
-   <span style="border-top: 1px">overlined</span> - Делает текст надчёркнутым
-   blink - Скрывает и показывает текст с интервалом
-   <span style="filter: invert(100%)">inverse</span> - Меняет местами цвета переднего плана и фона (псевдонимы: `swapcolors`, `swapColors`)
-   <span style="border-bottom: 1px double;">doubleunderline</span> - Делает текст дважды подчёркнутым (псевдоним: `doubleUnderline`)
-   <span style="border: 1px">framed</span> - Рисует рамку вокруг текста
...
-   `config` [`<Object>`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object) Используется для предоставления аргументов для разбора и для настройки парсера. `config` поддерживает следующие свойства:
    ...
    -   `allowPositionals` [`<boolean>`](https://developer.mozilla.org/docs/Web/JavaScript/Data_structures#Boolean_type) Принимает ли эта команда позиционные аргументы. **По умолчанию:** `false`, если `strict` равно `true`, иначе `true`.
