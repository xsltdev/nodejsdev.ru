...
: Мы можем использовать этот объект, чтобы убедиться, что родительский экземпляр был декорирован свойствами, которые мы используем в нашем плагине.
...
-   Мы устанавливаем минимальную версию Fastify на `5.x` (`[3]`).
-   Свойство `decorators` принимает ключи сущностей, которые могут быть декорированы — `fastify`, `request` и `reply`. Здесь мы проверяем, установлено ли у родительского экземпляра Fastify свойство `root` (`[4]`).
