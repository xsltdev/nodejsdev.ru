...
Все примеры кода в этой главе можно найти на [GitHub](https://github.com/PacktPublishing/Accelerating-Server-Side-Development-with-Fastify/tree/main/Chapter%201).
...
Компонентами утилиты, которые будут рассмотрены в [_главе 4_](./hooks.md), являются:
...
1.  `onRequest`: Сервер получает HTTP-запрос и направляет его на действительную конечную точку. Теперь запрос готов к обработке.
2.  `preParsing` происходит перед оценкой полезной нагрузки тела запроса.
3.  Хук `preValidation` запускается перед применением **JSON Schema validation** к частям запроса. Проверка схемы - важный этап каждого маршрута, поскольку он защищает вас от вредоносной полезной нагрузки запроса, направленной на утечку системных данных или атаку на ваш сервер. В [_главе 5_](./validation-serialization.md) этот основной аспект рассматривается более подробно, и будут показаны некоторые опасные атаки.
...
8.  `onResponse` выполняется после того, как HTTP-запрос был обслужен.
...
Корневой экземпляр приложения - это основной API, необходимый для создания вашего API. Все функции, управляющие запросами входящего клиента, должны быть зарегистрированы в нем, и это обеспечивает набор помощников, которые позволяют вам наилучшим образом организовать приложение. Мы уже рассмотрели, как создать его с помощью оператора `const app = fastify(serverOptions)`. Теперь мы представим общий обзор возможных вариантов настройки и использования этого объекта.
...
-   Параметр `logger` дает вам возможность адаптировать стандартный логгер к вашему удобству и системной инфраструктуре для архивирования распределенных логов и содержательных логов; в [_Главе 11_](../real-project/logging.md) будет подробно рассмотрено, как лучше настроить эти параметры.
-   `https: object` настраивает сервер на прослушивание сокетов **Transport Layer Security (TLS)**. Некоторые примеры мы увидим позже в [_главе 7_](../real-project/restful-api.md).
-   `keepAliveTimeout`, `connectionTimeout` и `http2SessionTimeout` - это несколько параметров таймаута, по истечении которого сокет HTTP-запроса будет уничтожен, освободив ресурсы сервера. Эти параметры передаются в стандартный Node.js `http.Server`.
-   Настройка маршрутизации для обеспечения более строгих или более слабых ограничений - например, URL без учета регистра и более детальный контроль для направления запроса к обработчику на основе дополнительной информации, такой как заголовок запроса вместо HTTP-метода и HTTP URL. Мы рассмотрим это в [_главе 3_](./routes.md).
...
-   Параметр `ajv` настраивает параметры проверки по умолчанию, чтобы улучшить соответствие вашей настройке. В [_Главе 5_](./validation-serialization.md) будет показано, как его использовать.
-   Функция `serverFactory: управляет создаваемым низкоуровневым HTTP-сервером. Эта функция является благословением при работе в бессерверной среде.
-   Настройки безопасности по умолчанию `onProtoPoisoning` и `onConstructorPoisoning` являются наиболее консервативными и обеспечивают безопасность приложения по умолчанию. Изменять их рискованно, и вы должны рассмотреть все вопросы безопасности, поскольку это влияет на стандартный парсер тела запроса и может привести к инъекции кода. В [_главе 4_](./hooks.md) будет показан пример этих параметров в действии.
...
#### Свойства экземпляра приложения {#application-instance-properties}
...
-   `app.initialConfig` для доступа к входной конфигурации в режиме только для чтения. Это будет удобно для плагинов, которым нужно читать конфигурацию сервера.
...
#### Методы экземпляра приложения {#application-instance-methods}
...
-   `app.register(plugin)` добавляет плагины к экземпляру сервера, при необходимости создавая новый контекст сервера. Этот метод обеспечивает Fastify **капсулирование**, о котором будет рассказано в [_главе 2_](./plugin-system.md).
...
-   `app.inject(options[, callback])` загружает сервер до достижения им состояния готовности и отправляет имитирующий HTTP-запрос. Об этом методе вы узнаете в [_главе 9_](../real-project/testing.md).

Это семейство API вернет нативный `Promise`, если вы не предоставите параметр обратного вызова. Этот шаблон кода работает для каждой функции, которую предоставляет Fastify: везде, где есть аргумент обратного вызова, вы можете опустить его и получить вместо него обещание!

Теперь у вас есть полное представление о компоненте экземпляра сервера Fastify и логике жизненного цикла, которую он реализует. Мы готовы использовать все, что прочитали до этого момента, и добавить в приложение наши первые конечные точки.
...
!!!note "Хук onRoute не работает?"
    ...
    Если хук `onRoute` ничего не показывает на консоли терминала, помните, что метод `addRoute` должен быть вызван после функции `addHook`! Вы заметили, какой характер может иметь хук: хуки в приложении синхронны и срабатывают при наступлении события, поэтому порядок кода для таких хуков имеет значение. Эта тема будет подробно рассмотрена в [_главе 4_](./hooks.md).
...
Вы видели, как легко добавляются новые маршруты, но можно ли сделать это еще более плавно? Да, может!
...
Функция-обработчик async может полностью избежать вызова метода `reply.send`. Она может возвращать полезную нагрузку напрямую. Для этого мы можем обновить конечную точку `GET /hello`:
...
В предыдущем коде мы установили таймер таймаута, который не держит цикл событий Node.js активным благодаря вызову `unref`. Если обратный вызов close не выполнится через 10 секунд, процесс завершится с ненулевым результатом. Этот паттерн реализован во многих плагинах, созданных сообществом Fastify, с которыми вы можете ознакомиться на странице _Ecosystem_ по адресу <https://www.fastify.io/ecosystem/>.
