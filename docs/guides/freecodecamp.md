...
При получении нового запроса вызывается функция `request` [event](../api/http.md#event-request), которая предоставляет два объекта: запрос (объект [`http.IncomingMessage`](../api/http.md#class-httpincomingmessage)) и ответ (объект [`http.ServerResponse`](../api/http.md#class-httpserverresponse)).
...
-   [**Express**](../frameworks/express.4/index.md) — Один из самых простых и в то же время мощных способов создания веб-сервера. Его минималистский подход и беспристрастное внимание к основным функциям сервера является ключом к его успеху.
...
Подробнее о кодах выхода вы можете прочитать [здесь](../api/process.md#exit-codes).
